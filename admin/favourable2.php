 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: favourable2.php 17217 2011-01-19 06:29:08Z liubo $
 */
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('href' => 'favourable2.php?act=add', 'text' => $_LANG['add_favourable']));
    ...
    /* 显示商品列表页面 */
    assign_query_info();
    $smarty->display('favourable_list2.htm');
}
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $id = intval($_GET['id']);
    $favourable = favourable_info2($id);
    if (empty($favourable))
    {
    ...
    $url = 'favourable2.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    else
    {
        ...
        if (isset($_POST['drop']))
        {
            ...
            $links[] = array('text' => $_LANG['back_favourable_list'], 'href' => 'favourable2.php?act=list&' . list_link_postfix());
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    assign_query_info();
    $smarty->display('favourable_info2.htm');
}
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $favourable = array(
        ...
        'act_type_ext'  => intval($_POST['act_type']) == 0 ? '' : join(',', $_POST['act_type_ext']),
        'gift'          => serialize($gift),
		'activity_type' => 2
    );
    ...
    {
        $links = array(
            array('href' => 'favourable2.php?act=add', 'text' => $_LANG['continue_add_favourable']),
            array('href' => 'favourable2.php?act=list', 'text' => $_LANG['back_favourable_list'])
        );
    ...
    {
        $links = array(
            array('href' => 'favourable2.php?act=list&' . list_link_postfix(), 'text' => $_LANG['back_favourable_list'])
        );
...
function favourable_list()
{
    ...
    {	
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('favourable_activity') .
                " WHERE 1 $where  AND activity_type = 2";
