...
if($_REQUEST['act'] == 'user_complain')
{
    ...
    /* 模板赋值 */
    $smarty->assign('ur_here', $_LANG['01_handle_complain']);
   ...
   $smarty->assign('complain_list',   $result['complain']);
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('user_complain.htm');
}

/*------------------------------------------------------ */
//-- 快钱订单列表
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'kuai_list')
{
	
	$smarty->display('kuai_list.htm');
}
...
elseif ($_REQUEST['act'] == 'delivery_info')
{
	...
	$aicha_expresses = get_aicha_expresses();
	
	$express_type = "aicha";
	$smarty->assign('expresses',$aicha_expresses);
    $smarty->assign('express_type',$express_type);
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    $action_note    = isset($_REQUEST['action_note']) ? trim($_REQUEST['action_note']) : '';
	 ...
	 $express_type    = isset($_REQUEST['express_type']) ? trim($_REQUEST['express_type']) : '';
	 $express_no    = isset($_REQUEST['express_no']) ? trim($_REQUEST['express_no']) : '';
	 $express_company    = isset($_REQUEST['express_company']) ? trim($_REQUEST['express_company']) : '';
	  $sql = "update ".$GLOBALS['ecs']->table('delivery_order')." set express_no = '$express_no',express_type = '$express_type', express_company='$express_company' where order_id='$order_id'";
...
/*------------------------------------------------------ */
//-- 管理员根据$type的值对用户投诉进行评论或删除 chen-0918
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'complain_info')
{
  ...
  if($complain_id == '') { echo "出现错误，投诉号为空"; exit;}
  
  $smarty->assign('complain',get_complain_detail($complain_id));
 $smarty->display('complain_info.htm');exit;
}
...
elseif ($_REQUEST['act'] == 'back_info')
{
 ...
 $order = order_info($back_order['order_id']);
    /* 模板赋值 */
	if($order['goods_amount'] < $back_order['return_money'])
	  {
	       $smarty->assign('agree', 1);
		   
	  }
	  /*取得用户退换货时提交的图片*/
	 $order_id = $back_order['order_id'];
	 $sql = 'select pic_url from '.$GLOBALS['ecs']->table('back_order_pic').' where order_id='.$order_id;
	// echo $sql;
	 $back_pics = $GLOBALS['db']->getAll($sql);
	 $smarty->assign('back_pics',$back_pics);
	//  var_dump($back_pics);
	 $back_order['address'] = $order['address'];
	  $smarty->assign('order', $order);
...
elseif ($_REQUEST['act'] == 'admin_hander_back_order')
{
$agree_type = $_REQUEST['agree_type'];
$back_id = $_REQUEST['back_id'];

$sql = 'select back_id from '.$GLOBALS['ecs']->table('back_order_reply').' where back_id='.$back_id;
$flag=$GLOBALS['db']->getOne($sql);
if($flag)
{
$sql = "update ".$GLOBALS['ecs']->table('back_order_reply')." set reply_time=".gmtime().",action_type=".$agree_type;
}
else
{
$sql = 'insert into '.$GLOBALS['ecs']->table('back_order_reply').' (back_id,reply_time,action_type) values('.$back_id.','.gmtime().','.$agree_type.')';
}
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    elseif (isset($_POST['unpay']))
    {
	   ...
	   /*一键发货*/
     elseif (isset($_POST['fast_deliver']))
	 {
        ...
        if ($goods_list)
        {
            foreach ($goods_list as $key=>$goods_value)
            {
                ...
                if (($goods_value['extension_code'] == 'package_buy') && (count($goods_value['package_goods_list']) > 0))
                {
                    ...
                    "&operation=lack_goods&action_note=" . urlencode($action_note) . "\n");
                      exit; 
                        }
                        /* 将已经全部发货的商品设置为只读 */
	...
	//	var_dump($order);
       ...
       // $action_note = isset($_REQUEST['action_note']) ? trim($_REQUEST['action_note']) : '';
        ...
        $deliver_goods_list = $_goods['goods_list'];
//var_dump($deliver_goods_list);
        /* 检查此单发货数量填写是否正确 合并计算相同商品和货品 */
        if (!empty($send_number) && !empty($deliver_goods_list))
        {
            $goods_no_package = array();
            foreach ($deliver_goods_list as $key => $value)
            {
                /* 去除 此单发货数量 等于 0 的商品 */
				...
				//	var_dump($value['rec_id']);
 //var_dump($send_number);
                    //去除
                    if ($send_number[$value['rec_id']] <= 0)
                    {
                        unset($send_number[$value['rec_id']], $deliver_goods_list[$key]);
                        continue;
                    }
                ...
                else
                {
                    ...
                    /* 组合超值礼包信息 */
                    $deliver_goods_list[$key]['package_goods_list'] = package_goods($value['package_goods_list'], $value['goods_number'], $value['order_id'], $value['extension_code'], $value['goods_id']);
                    ...
                    foreach ($value['package_goods_list'] as $pg_key => $pg_value)
                    {
                        ...
                        {
                            unset($send_number[$value['rec_id']][$pg_value['g_p']], $deliver_goods_list[$key]['package_goods_list'][$pg_key]);
                        }
                    }

                    if (count($deliver_goods_list[$key]['package_goods_list']) <= 0)
                    {
                        unset($send_number[$value['rec_id']], $deliver_goods_list[$key]);
                        continue;
                    }
                ...
                {
                    /* 超值礼包 */
                    foreach ($deliver_goods_list[$key]['package_goods_list'] as $pg_key => $pg_value)
                    {
...
//var_dump($delivery);
	//	exit;
        ...
        if ($delivery_id)
        {
            ...
            //发货单商品入库
            if (!empty($deliver_goods_list))
            {
                foreach ($deliver_goods_list as $value)
                {
                    // 商品（实货）（虚货）
        ...
        if (true)
        {
            ...
            foreach ($deliver_goods_list as $key => $value)
            {
                if ($value['extension_code'] == 'package_buy')
                {
                    unset($deliver_goods_list[$key]);
                }
            }
            $_goods['goods_list'] = $deliver_goods_list + $_goods['goods_list'];
            unset($deliver_goods_list);
 ...
 //$links[] = array('text' => $_LANG['order_info'], 'href' => 'order.php?act=info&order_id=' . $order_id);
   ...
   //   sys_msg($_LANG['act_ok'] . $msg, 0, $links);
	 ...
	 ecs_header("Location:order.php?act=delivery_info&delivery_id=$delivery_id\n");
	 exit;
...
function complain_list()
{
   $result = get_filter();
    if ($result === false)
    {
        ...
        $filter['time'] = empty($_REQUEST['time']) ? '' : trim($_REQUEST['time']);
        $filter['user_name'] = empty($_REQUEST['user_name']) ? 0 : intval($_REQUEST['user_name']);
     $filter['type'] = empty($_REQUEST['type']) ? 0 : intval($_REQUEST['type']);
	 $filter['state'] = empty($_REQUEST['state']) ? 0 : intval($_REQUEST['state']);

        $filter['sort_by'] = empty($_REQUEST['sort_by']) ? 'time' : trim($_REQUEST['sort_by']);
        $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);

      
	     $where = 'WHERE is_del = 0 '; //修改where条件 chen 0901
 ...
 /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('complain') . $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;

  $sql = "SELECT * from " . $GLOBALS['ecs']->table('complain') . "
                $where
  ...
  $row = $GLOBALS['db']->getAll($sql);
    ...
    foreach ($row AS $key => $value)
    {
        ...
        $row[$key]['user_name'] = $value['user_name'];//$GLOBALS['db']->getOne($sql); //$value['id'];//'id:'.$value['id']."   name:".$GLOBALS['db']->getOne($sql);//
		 $row[$key]['order_sn'] = $value['order_sn'];
		switch($value['state'])
		{
		  case 0:$row[$key]['state'] = '未处理';break;
		  case 1:$row[$key]['state'] = '处理中';break;
		  case 2:$row[$key]['state'] = '已处理';break;
		}
	
        $row[$key]['time'] = local_date("Y-m-d H:i:s",$value['time']);
		$row[$key]['order_id'] = $value['order_id'];
        $row[$key]['type'] = $value['type'] == 0 ? "投诉" : "建议";
    }
    $arr = array('complain' => $row, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
...
function back_list() 
{
    ...
    if ($result === false)
    {
       ...
       // $where = 'WHERE 1 '; 
	     $where = 'WHERE is_cancled = 0 '; //修改where条件 chen 0901
        if ($filter['order_sn'])
        {
            $where .= " AND order_sn LIKE '%" . mysql_like_quote($filter['order_sn']) . "%'";
        }
...
/*根据投诉id，获取投诉内容 chen-0919*/
function get_complain_detail($complain_id)
{
   $sql = "select * from ".$GLOBALS['ecs']->table('complain') . " where id = $complain_id";
   $row = $GLOBALS['db']->getRow($sql);
   $row['time'] = local_date("Y-m-d H:i:s",$row['time']);
   $row['type'] = $row['type'] == 0 ? '投诉' : '建议';
   switch ($row['state'])
   {
   case 0: $row['state'] = '未处理'; break;
   case 0: $row['state'] = '处理中'; break;
   case 0: $row['state'] = '已处理'; break;
   }
   
   $sql = "select content from ".$GLOBALS['ecs']->table('reply_complain')." where complain_id=$complain_id order by time desc";
   $row['reply'] = $GLOBALS['db']->getOne($sql);
   $row['reply'] = $row['reply'] ? $row['reply'] : "未处理，请在下方文本框内填写回复内容。多次提交，将会取最新的一次回复作为最后的回复。";
   return $row;
}
/*客服回复 用户的投诉/建议  chen-0919*/
function update_reply_content($complain_id,$content,$type)
{
if($type == 'delete')
{
  $sql = "update ".$GLOBALS['ecs']->table('complain')." set is_del = 1 where id = $complain_id";
  $GLOBALS['db']->query($sql);
return ;
}
else if($type != 'update')
{
  echo "error,type value is invaild"; return;
}
...
if(empty($content)) return ;
  ...
  $sql = "select id from ".$GLOBALS['ecs']->table('reply_complain')." where complain_id=$complain_id";
  $flag = $GLOBALS['db']->getOne($sql);
  ...
  if($flag)
  {
    $sql = "update ".$GLOBALS['ecs']->table('reply_complain')." set	content = '$content' , admin_id = $admin_id , time = $time where complain_id = $complain_id";
  }
  else
  { 
    $sql = "update ".$GLOBALS['ecs']->table('complain')." set state = 2 where id = $complain_id";  //将投诉表对应的用户投诉状态标记为：已处理
    $GLOBALS['db']->query($sql);
	
    $sql = "insert into ".$GLOBALS['ecs']->table('reply_complain')." (content,admin_id,time,complain_id) values('$content',$admin_id,$time,$complain_id)";
