 ...
 * @return  array
 */
function sort_flag_bootstrap($filter)
{
    $flag['tag']    = 'sort_' . preg_replace('/^.*\./', '', $filter['sort_by']);
    $flag['img']    =  '<span class="glyphicon glyphicon-triangle-'.($filter['sort_order'] == "DESC" ? 'bottom' : 'top') .'" aria-hidden="true">';
...
function bootstrap_pagination($filter)
{
    $pagination = ' <ul class="pagination" style="float:right;margin:10px 0 0 0; "> <li><a href="javascript:listTable.gotoPage(1)">首页</a></li><li><a href="javascript:listTable.gotoPagePrev()">&laquo;</a></li>';
    
         $start = ($filter['page'] - 2) > 1 ? ($filter['page'] - 2) : 1;
         $end = ($filter['page'] + 2) < $filter['page_count'] ? ($filter['page'] + 2) : $filter['page_count'];
       ...
       for($i = $start; $i <= $end; $i++  )
       {
         ...
         {
            $pagination .=  '<li><a href="javascript:listTable.gotoPage('.$i.')">'.$i.'</a></li>';
         }
       }
    $pagination .= '<li><a href="javascript:listTable.gotoPageNext()">&raquo;</a></li>';
    $pagination .= '<li><a href="javascript:listTable.gotoPage('.$filter['page_count'].')">末页</a></li>';
    $pagination .= '</ul>';
    return $pagination;
}
?>