...
function handle_vip_member_price($goods_id, $rank_list, $price_list, $integral_list)
{
    /* 循环处理每个会员等级 */
    foreach ($rank_list AS $key => $rank)
    {
        /* 会员等级对应的价格 */
        $price = $price_list[$key];
         $integral = $integral_list[$key];
        // 插入或更新记录
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('vip_member_price') .
               " WHERE goods_id = '$goods_id' AND user_rank = '$rank'";
        if ($GLOBALS['db']->getOne($sql) > 0)
        {
            /* 如果会员价格是小于0则删除原来价格，不是则更新为新的价格 */
            if ($price < 0)
            {
                $sql = "DELETE FROM " . $GLOBALS['ecs']->table('vip_member_price') .
                       " WHERE goods_id = '$goods_id' AND user_rank = '$rank' LIMIT 1";
            }
            else
            {
                $sql = "UPDATE " . $GLOBALS['ecs']->table('vip_member_price') .
                       " SET user_price = '$price' , user_integral = '$integral' " .
                       "WHERE goods_id = '$goods_id' " .
                       "AND user_rank = '$rank' LIMIT 1";
            }
        }
        else
        {
            ...
            {
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('vip_member_price') . " (goods_id, user_rank, user_price,user_integral) " .
                       "VALUES ('$goods_id', '$rank', '$price','$integral')";
            }
        }
...
{
    $goods_sn = $GLOBALS['_CFG']['sn_prefix'] . str_repeat('0', 5 - strlen($goods_id)) . $goods_id;
...
//preg_match_all("",$body,$img_array);

preg_match_all("/<[img|IMG].*?src=[\'|\"](.*?(?:[\.gif|\.jpg]))[\'|\"].*?[\/]?>/",$body,$img_array); 
