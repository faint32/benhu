...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT DISTINCT goods_id, goods_sn, goods_name, market_price, shop_price, goods_number, integral, give_integral, brand_id, is_real,goods_yh FROM " . $ecs->table('goods') . $where;
   //var_dump($db->getAll($sql));
  /* $arr = $db->getAll($sql);
   for($i=0; $i<count($arr);$i++)
	{
     $arr[$i] = addslashes($arr[$i]['goods_yh']);
   }
   var_dump($arr);
   */
        ...
        if (!empty($_POST['goods_id']))
        {
            foreach ($_POST['goods_id'] AS $goods_id)
            {
                ...
                $goods = array(
                    ...
                    'last_update'   => gmtime(),
					'goods_yh'      => $_POST['goods_yh'][$goods_id]
                );
    ...
    else
    {
        ...
        if (!empty($_POST['goods_id']))
        {
            foreach ($_POST['goods_id'] AS $goods_id)
            {
				...
				if (trim($_POST['goods_yh']) != '')
				{
				    $goods['goods_yh'] = $_POST['goods_yh'];
				}
                if (!empty($goods))
                {
                    $db->autoExecute($ecs->table('goods'), $goods, 'UPDATE', "goods_id = '$goods_id'");
