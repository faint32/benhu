...
if ($_REQUEST['act']=='reply')
{
	...
	if($comment_info['comment_type']==0  && $comment_info['order_id']!='' ) //获取晒单图片
	{
	  $sql = "select thumb from ".$GLOBALS['ecs']->table('shaidan')." where order_id=".$comment_info['order_id']." and goods_id=".$comment_info['id_value'];
       $thumbs = $GLOBALS['db']->getAll($sql);	
	   $comment_info['thumbs'] = $thumbs;
	}
