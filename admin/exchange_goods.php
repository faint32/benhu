...
if ($_REQUEST['act'] == 'list')
{
    ...
    update_exchange_goods(); //如果下期预告的时间已经到了，将其更新为精品上新。
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('exchange_goods')."(goods_id, exchange_integral, is_exchange, is_best,is_new,is_next,begin_time) ".
            "VALUES ('$_POST[goods_id]', '$_POST[exchange_integral]', '$_POST[is_exchange]', '$_POST[is_best]', '$_POST[is_new]', '$_POST[is_next]', '$bg_time')";
			//echo $sql;
    $db->query($sql);
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT eg.goods_id, eg.exchange_integral,eg.is_exchange,eg.is_new,eg.is_best,eg.is_next,eg.begin_time,eg.needed_money, g.goods_name ".
...
if ($_REQUEST['act'] =='update')
{
    ...
    $bg_time=local_strtotime($_POST['start_time']);
    if ($exc->edit("exchange_integral='$_POST[exchange_integral]', is_exchange='$_POST[is_exchange]', is_new='$_POST[is_new]', is_best='$_POST[is_best]',is_next='$_POST[is_next]',begin_time='$bg_time', needed_money='$_POST[needed_money]' ", $_POST['goods_id']))
    {
...
/*如果下期预告的时间已经到了，将其更新为精品上新。*/
function update_exchange_goods()
{
$now = gmtime();
$sql = "update ".$GLOBALS['ecs']->table('exchange_goods')." set is_new = 1,is_next=0 where begin_time < $now and is_next=1";
