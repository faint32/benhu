...
listTable.bootstrap_toggle = function(obj, act, id)
{
  var val = ($(obj).attr('status').match(/yes/i)) ? 0 : 1;
  ...
  if (res.error == 0)
  {
    ...
    $(obj).addClass( (res.content > 0) ?   'glyphicon-ok' : 'glyphicon-remove' );
    $(obj).attr('status',(res.content > 0) ? 'yes' : 'no');
  }
}
