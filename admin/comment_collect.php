...
if ($_REQUEST['act'] == 'comment')
{
	...
	$smarty->assign('goodsPage', $_REQUEST['goodsPage']);
...
elseif ($_REQUEST['act'] == 'comment_preview')
{
	...
	foreach($comment_list as $key=>$val)
	{
		...
		{
			$time = local_strtotime(str_replace('.','-',$dateList[$key]));
		}
		else
		{
			$time = gmtime();
...
elseif ($_REQUEST['act'] == 'comment_batch_import')
{
	...
	if($arrdata['result'] == 'success')
	{
		...
		foreach($arrdata['data'] as $comment)
		{
			...
			{			
				...
				$buy_time = intval($_POST['buy_time']);		
				$password=time();	
				$reg_date = local_strtotime($_POST['add_time'][$comment['id']])-30*24*3600;	
				$ip = '';				
				/*
				...
				*/	
				if(check_user($user_name) == false)  //edit by cy
				{
					  $rankPoint = getRandRankPoint();	
					 
			$sql = 'INSERT INTO ' . $GLOBALS['ecs']->table("users") . "(`email`, `user_name`, `password`, `reg_time`, `last_login`, `last_ip`, `rank_points`) VALUES ('$email', '$user_name', '$password', '$reg_date', '$reg_date', '$ip', '$rankPoint')";					  
				$db->query($sql);					
				}
			$userid = $db->getOne("SELECT user_id FROM " . $GLOBALS['ecs']->table("users") . " WHERE user_name = '" . $user_name . "'");
				
				$sql = "INSERT INTO " .$ecs->table('comment') ."(comment_type, id_value, email, user_name, content, comment_rank, add_time, ip_address, status, parent_id, user_id) VALUES " ."('" .$comment_type. "', '" .$id_value. "', '$email', '$user_name', '" .$content."', '".$comment_rank."', '".$add_time."', '".$ip_address."', '$status', '$parent_id', '$userid')";		
				$db->query($sql);	
				...
				$shipping_status = 2;	
				$pay_status = 2;	
				$addtime = local_strtotime($_POST['add_time'][$comment['id']]);


                if(empty($buy_time)) //随机购买时间，具体为3-6天
                {
	               $pay_time = local_strtotime($_POST['add_time'][$comment['id']])-rand(3*24*3600,6*24*3600);
                }
                else
                {
                	$pay_time = local_strtotime($_POST['add_time'][$comment['id']])-
                	rand(($buy_time - 1)*24*3600, $buy_time*24*3600);
                }
				...
				$db->query('INSERT INTO ' . $GLOBALS['ecs']->table("order_info") . 
						"(`order_sn`, `user_id`, `order_status`, `shipping_status`, `pay_status`, `add_time`, `pay_time`) 
						VALUES ('$order_sn', '$userid', '$order_status', '$shipping_status', '$pay_status', '$xxc', '$pay_time')");
	...
	$sql = "SELECT brand_id FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id=".$_GET['goods_id'];
 	$brand_id = $GLOBALS['db']->getOne($sql);
 	$goodsPage = $_REQUEST['goodsPage'];

	$link[] = array('href' => 'goods.php?act=list&brand_id='.$brand_id.'&goodsPage='.$goodsPage, 'text' => "进入商品列表");
...
{   
 ...
 function getRandRankPoint()
 {
	$sql = "SELECT min_points FROM ".$GLOBALS['ecs']->table('user_rank')." WHERE rank_name LIKE 'VIP%'";
	$minPoints = $GLOBALS['db']->getAll($sql);
	
	  $rnd = rand(1,100);
	  
	    if($rnd <= 40)
		{
		   $rankPoint = $minPoints[0];
		}
		else if($rnd <= 70)
		{
			$rankPoint = $minPoints[1];
		}
		else if($rnd <= 85)
		{
			$rankPoint = $minPoints[2];
		}
		else
		{
		   $rankPoint = $minPoints[3];
		}
		return $rankPoint['min_points'];
 }
