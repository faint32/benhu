...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    else
    {
        ...
        if (!empty($goods['goods_yh']))
        {
            $goods['goods_yh'] = $goods['goods_yh'];
        }
        if (!empty($goods['is_age']))
        {
            $goods['is_age'] = $goods['is_age'];
        }
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($_POST['goods_name'])
    {
        $sql = "SELECT COUNT(*) FROM " . $ecs->table('goods') .
                " WHERE goods_name = '$_POST[goods_name]' AND is_delete = 0 AND goods_id <> '$_POST[goods_id]'";
       
        if ($db->getOne($sql) > 0)
        {
            sys_msg($_LANG['goods_name_exists'], 1, array(), false);
        }
    }
	...
	//$_POST[goods_desc]=GetCurContent($_POST[goods_desc]);
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief,goods_yh,is_age, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[goods_yh]','$_POST[is_age]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
...
elseif ($_REQUEST['act'] == 'edit_goods_name')
{
    ...
    /* 检查是否重复 */
    if (!$exc->is_only('goods_name', $goods_sn, $goods_id))
    {
        make_json_error($_LANG['goods_name_exists']);
    }
    $sql="SELECT goods_id FROM ". $ecs->table('goods')."WHERE goods_name='$goods_name' AND is_delete = 0";
    if($db->getOne($sql))
    {
        //make_json_error($_LANG['goods_name_exists']);
    }
...
elseif ($_REQUEST['act'] == 'check_goods_name')//检测名称是否重复
{
    check_authz_json('goods_manage');

    $goods_id = intval($_REQUEST['goods_id']);
    $goods_name = htmlspecialchars(json_str_iconv(trim($_REQUEST['goods_name'])));
...
elseif ($_REQUEST['act'] == 'product_list')
{
    ...
    if (empty($goods))
    {
     ...
     $smarty->assign('name', sprintf($_LANG['good_goods_name'], $goods['goods_name']));
