...
'---------------------------------------------------------------------------------------------
	...
	'===============================================================================
	 ...
	 Upload.Exe="bmp|png|jpeg|jpg|gif|mp3"          			'设置合法扩展名,以|分割,忽略大小写
	...
	'===============================================================================
	 if Upload.ErrorID>0 then								'判断错误号,如果myupload.Err<=0表示正常
		...
		if Upload.files(-1).count>0 then 					'这里判断你是否选择了文件
				path=server.mappath(".") 					'文件保存路径(这里是当前文件夹)
				...
				Response.Write("<br />上传成功！你选择的是<font color='#ff0000'>"&Upload.forms("select")&"</font>--<font color='#0000ff'>"&Upload.forms("select2")&"</font>")
				...
				response.Write(" <br />旋转角度："&Trim(Upload.forms("access2008_image_rotation")))
...
function getCookie(value) '获取cookie
  ...
  getCookie = Upload.forms("access2008_cookie_"&value)
...
sub outMP3Info()
	...
	if len(info)>0 then
		...
		if ubound(infoArr) = 8 then
		  ...
		  elseif infoArr(5) = 1 then
			  response.Write("声道模式:Joint Stereo<br/>")
		  elseif infoArr(5) = 2 then
      ...
      response.Write("播放时长:"&(int(infoArr(8))/1000)&"秒<br/>")
