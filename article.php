...
if (!$smarty->is_cached('article.dwt', $cache_id))
{
    ...
    $smarty->assign('cat_id',           $cat_id);//0828-guan
    ...
    $position = assign_ur_here($article['cat_id'], $article['title']);//0828-guan修改了页面的所在位置样式
...
function isBabyNurtureArticle($articleId)
{
     $cat_id = $GLOBALS['db']->getOne("SELECT cat_id FROM " . $GLOBALS['ecs']->table('article') . " WHERE article_id = '".$articleId."' ");
     $babyNurtureArticleCateIds = getBabyNurtureArticleCateIds();
     if(in_array($cat_id, $babyNurtureArticleCateIds))
     {
        return true;
     }
     else
...
function getBabyNurtureArticleCateIds()
{
   $babyNurtureName = "育儿知识";
   $babyNurtureCateId = $GLOBALS['db']->getOne("SELECT cat_id FROM " . $GLOBALS['ecs']->table('article_cat') . " WHERE cat_name = '".$babyNurtureName."' ");
   $babyNurtureCateIds = array($babyNurtureCateId['cat_id']);

    $tmp = $GLOBALS['db']->getAll("SELECT cat_id FROM " . $GLOBALS['ecs']->table('article_cat') . " WHERE parent_id = '".$babyNurtureCateId."' ");
    foreach ($tmp as  $value) {
       $babyNurtureCateIds[] = $value['cat_id'];
    }
    return $babyNurtureCateIds;
}

function getArticleComments($article_id)
{
    $sql = "SELECT user_name,  content, comment_rank, parent_id, add_time FROM ".$GLOBALS['ecs']->table('comment')." WHERE comment_type='1' AND id_value='$article_id' AND status='1' ORDER BY comment_id DESC, add_time DESC";
    $comments = $GLOBALS['db']->getAll($sql);

    foreach ($comments as $key => $value) {
        $comments[$key]['add_time'] = local_date("Y-m-d H:i:s",$comments[$key]['add_time']);
        if(!empty($value['user_name']))
        {
            $sql = "SELECT nickname, personal_pic FROM ".$GLOBALS['ecs']->table('users')." WHERE user_name='".$value['user_name']."'";
            $row = $GLOBALS['db']->getRow($sql);
            $comments[$key]['user_name'] = $row['nickname'];
            $comments[$key]['personal_pic'] = $row['personal_pic'];
        }
    }
