...
if($_REQUEST['act'] == 'change')
{
   ...
   if($group_type=='today')
   {
   $sql[0]= $comm . " AND b.start_time <= '$now' AND b.end_time > '$now' and group_buy_type=0  and brand_buy=0 AND b.is_finished < 3 ORDER BY RAND() LIMIT 8";
   $sql[1]= $comm . " AND b.start_time <= '$now' AND b.end_time > '$now' and group_buy_type=1  and brand_buy=0 AND b.is_finished < 3 ORDER BY RAND() LIMIT 8";
   }
   else if($group_type=='brand')
   {
   $sql[0]=  $comm . " AND b.start_time <= '$now' AND b.end_time > '$now' and group_buy_type=0  and brand_buy=1 AND b.is_finished < 3 ORDER BY RAND() LIMIT 8";
   $sql[1]=  $comm . " AND b.start_time <= '$now' AND b.end_time > '$now' and group_buy_type=1  and brand_buy=1 AND b.is_finished < 3 ORDER BY RAND() LIMIT 8";
   }
   else if($group_type=='not_start')
   {
   $sql[0]=  $comm . " AND b.start_time > '$now'  and group_buy_type=0   AND b.is_finished < 3 ORDER BY RAND() LIMIT 8";
   $sql[1]=  $comm . " AND b.start_time > '$now'  and group_buy_type=1   AND b.is_finished < 3 ORDER BY RAND() LIMIT 8";
   }
   $arr[0]=$group_type;
   $arr[1]=group_buy_list($sql[0]);
   $arr[2]=group_buy_list($sql[1]);
...
else if ($_REQUEST['act'] == 'list')
{
    ...
    {
	    /*取得主广告和右广告*/
		   $group_main_ad = $db->getAll('SELECT ad_code, ad_link FROM ' . $ecs->table("ad") . ' WHERE position_id = 12 LIMIT 1 ');
		   $smarty->assign('group_main_ad', $group_main_ad[0]);
		
		   $group_right_ad = $db->getAll('SELECT ad_code, ad_link FROM ' . $ecs->table("ad") . ' WHERE position_id = 13 LIMIT 1 ');
		   $smarty->assign('group_right_ad', $group_right_ad[0]);
        ...
        if ($count > 0)
        {
         ...
         $now = gmtime();
                ...
                $comm = "SELECT b.*,g.market_price, IFNULL(g.goods_thumb, '') AS goods_thumb, g.shop_price,b.act_id AS group_buy_id, ". "b.start_time AS start_date, b.end_time AS end_date " . "FROM " . $GLOBALS['ecs']->table('goods_activity') . " AS b " ."LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ON b.goods_id = g.goods_id  WHERE b.act_type = '" . GAT_GROUP_BUY."'";
           $comm2 = " b.is_finished < 3 ORDER BY RAND() LIMIT 8";
		   $where_start_time['lt']=" b.start_time <= '$now' ";
		   $where_end_time['gt']=" b.end_time > '$now' ";
		   $where_start_time['gt']=" b.start_time  > '$now' ";
		   $where_type['mum']=" group_buy_type=0 "; //妈妈专区
		   $where_type['baby']=" group_buy_type=1 ";
		   $where_brand[0]=" brand_buy=0 ";    //非品牌团购
		...
		//$smarty->assign('group_main_ad', $group_main_ad);     //团购页主广告
