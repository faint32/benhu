...
//chen-0925
if($_REQUEST['act'] == 'ajax_change_goods')
{
 $goods = get_collected_goods($goods_id);
  echo json_encode($goods);
  exit;
}


if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'choose_attr')
{
    include('includes/cls_json.php');
    $json   = new JSON;
    $good_id   = $_REQUEST['good_id'];
   
   $sql = "select goods_id,goods_name,goods_number,market_price,shop_price from ".$GLOBALS['ecs']->table('goods')." where goods_id='$good_id'";
    $good = $GLOBALS['db']->getRow($sql);
   
   $sql = "SELECT g.goods_attr_id,a.attr_name, g.attr_id,g.attr_value, g.attr_price " .
        'FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' AS g ' .
        ' LEFT JOIN '.$GLOBALS['ecs']->table('attribute').' AS a ON g.attr_id = a.attr_id '.
        "WHERE g.goods_id = '" . $good_id . "' " .
        'ORDER BY  g.attr_id,g.attr_price';
 $attr = $GLOBALS['db']->getAll($sql);

   $good['attr'] = $attr;
    die($json->encode($good));
}
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    if (is_spec($attr_id) && !empty($prod))//此处是动态加载计算购买后的商品库存量
    {
        $product_info = get_products_info($goods_id, $attr_id);
    }
    if ($product_info['product_number'])
    {
        $res['stock'] = $product_info['product_number'];//商品库存
    }else{
        $res['stock'] = $goods_tinfo['goods_number'];//商品库存
    }
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    goods_promotion($goods_id,$smarty);   // 积分购,vip购
    ...
    else
    {
        ...
        foreach($linked_goods as $kLinkGood => $vGood )
         {
                  ...
                  $linked_goods[$kLinkGood]['url'] = build_uri('goods', array('gid' => $linked_goods[$kLinkGood]['goods_id']), $linked_goods[$kLinkGood]['goods_name']); 
                   $linked_goods[$kLinkGood]['shop_price'] = round( $linked_goods[$kLinkGood]['shop_price'], 2);      
         }
        ...
        $smarty->assign('goodsyh',          $goodsyh);                                           // 商品优惠信息
        ...
        $smarty->assign('pictures',            get_goods_gallery($goods_id));                    // 商品相册->原版调取商品图片
        ...
        assign_rank_name($smarty); //chen-0925 添加函数assign_rank_name
...
//查询库存量，商品库存量-已经发出的货物数量
$sql = 'SELECT g.goods_number as num1, o.goods_number as num2 FROM ' . $ecs->table('goods') . ' AS g LEFT JOIN ' . $ecs->table('order_goods') . " AS o ON g.goods_id = o.goods_id WHERE  g.goods_id= ".$goods_id;
$goods_nums=$db->getAll($sql);
$nums=$goods_nums[0]['num1']-$goods_nums[0]['num2'];
$smarty->assign('nums',  $nums);           // 当前系统时间
...
//-- 获取用户等级名称chen-0925
/*------------------------------------------------------ */
function assign_rank_name($smarty)
{
  $sql = "select rank_name from " . $GLOBALS['ecs']->table('user_rank') . ' where rank_id = ' . $_SESSION['user_rank'];
  $rank_name = $GLOBALS['db']->getOne($sql);
  $smarty->assign('rank_name' , $rank_name);
}

function goods_promotion($goods_id , $smarty)  
{
$now = gmtime();
$sql = "select needed_money , exchange_integral, goods_id from ".$GLOBALS['ecs']->table('exchange_goods')." where goods_id='$goods_id' and begin_time < $now and is_exchange = 1";
 $exchangePromotion = $GLOBALS['db']->getRow($sql);
 if($exchangePromotion)
$smarty->assign('exchangePromotion',$exchangePromotion);

    if($_SESSION['user_rank'] > 0)
    {
    $sql = "select goods_id from ".$GLOBALS['ecs']->table('vip_goods')." where goods_id='$goods_id' and begin_time < $now and is_exchange = 1";
        if($GLOBALS['db']->getRow($sql))
        {
        $sql = "select goods_id,user_price,user_integral from ".$GLOBALS['ecs']->table('vip_member_price')." where goods_id='$goods_id' and user_rank = ".$_SESSION['user_rank'];
        $vipPromotion = $GLOBALS['db']->getRow($sql);
        if($vipPromotion)
$smarty->assign('vipPromotion',$vipPromotion);
        }
    }
  }
/**
...
function get_collected_goods($goods_id)
{
  ...
  $sql = "select distinct oi.user_id from " . $GLOBALS['ecs']->table('order_info') . " as oi join ".
          $GLOBALS['ecs']->table('order_goods')." as og ".
          " on oi.order_id = og.order_id ".
          " where og.goods_id = $goods_id order by rand() limit 4";
...
function get_also_bought($goods_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$key]['goods_num']    = get_goods_buy_sum($row['goods_id']);
        if ($row['promote_price'] > 0)
        {
