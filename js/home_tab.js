   ...
   (function($){
	...
	$("#banner-slide1").slider({  //chen-0915
        interval:5000,
        loop: true,
        auto:false
    });
	 $("#banner-slide2").slider({  //chen-0915
        interval:5000,
        loop: true,
        auto:false
    });
...
/*-----------------------------------*/
   ...
   (function($){
        $.fn.slider=function(arg){
            var me = $(this),opt = $.extend({inteval:5000,auto:true,loop:false,prev:me.find(".prev_2"),next:me.find(".next_2"),pageBtns:"",finish:function(){},start:function(){}}, arg),pageBtns,scrollable = me.find(".scrollable_2"),ori_items = me.find(".items_2"),total_num = me.find(".item_2").length,cur_page = 0,mov_w = me.find(".item_2").outerWidth(),move_left=0,interval;
            opt.start();
            if (opt.loop) {
                ori_items.append(ori_items.find(".item_2").clone());
                ori_items.prepend(ori_items.find(".item_2").clone());
                scrollable.scrollLeft(getCurScroLeft());
            }
           opt.prev.length&&opt.prev.bind("click",move).hover(function(){$(this).addClass("hover_2")},function(){$(this).removeClass("hover_2")});
           opt.next.length&&opt.next.bind("click",move).hover(function(){$(this).addClass("hover_2")},function(){$(this).removeClass("hover_2")});
           if(opt.auto){
               interval=setInterval(move,opt.inteval);
               me.hover(function(){clearInterval(interval)},function(){interval=setInterval(move,opt.inteval);move_left=0});
           }
           ...
           function moveTo(e){
                pageBtns.unbind("click",moveTo);
                var cur_li=$(e.target).closest("li"),num=cur_li.index();
                pageBtns.removeClass("select_2");cur_li.addClass("select_2");
                var cur_scrollLeft=num*mov_w;
                scrollable.animate({scrollLeft:cur_scrollLeft},{duration:'normal',easing:'swing',complete:function(){pageBtns.bind("click",moveTo);opt.finish(num)},queue:false});
           }
           function move(e){
              ...
              if(e){
                  clearInterval(interval);
                  var cur_btn=$(e.target);
                  if(cur_btn.hasClass("prev_2")){move_left=1};
                  if(cur_btn.hasClass("next_2")){move_left=0};
              }
               var cur_scrollLeft=move_left ? getCurScroLeft()-mov_w:getCurScroLeft()+mov_w;
               scrollable.stop().animate({scrollLeft:cur_scrollLeft},{duration:'normal',easing:'swing',complete:complate,queue:false});
           }
