...
function validateCouponsResponse(obj)
{
 ...
 function changeNeedInv()
{
  ...
  var obj        = document.getElementById('ECS_NEEDINV');  //input:checkbox 开发票选择框
  var objType    = document.getElementById('ECS_INVTYPE');
  var objPayee   = document.getElementById('ECS_INVPAYEE');
  var objContent = document.getElementById('ECS_INVCONTENT');
  
  
  var needInv    = obj.value == 0 ? 0 : 1;

  var invType    = (needInv == 1) ? (objType != undefined ? objType.value : '') : '';
  var invPayee   = (needInv == 1) ? objPayee.value : '';
  var invContent = (needInv == 1) ? objContent.value : '';
  objType.disabled = objPayee.disabled = objContent.disabled = ! needInv;
  if(objType != null)
  {
    objType.disabled = ! needInv;
  }

  Ajax.call('flow.php?step=change_needinv', 'need_inv=' + needInv + '&inv_type=' + encodeURIComponent(invType) + '&inv_payee=' + encodeURIComponent(invPayee) + '&inv_content=' + encodeURIComponent(invContent), changeNeedInvCallBack, 'GET');
}
function changeNeedInvCallBack(result)
{
result = eval( "(" + result + ")" );
if(result['need_inv']==1)
{
$('#tax_fee').html('¥'+result['tax_formated'])
$('#fapiao_dsp').css("display","block");
}
else
{
$('#fapiao_dsp').css("display","none");
}
$('#total_amount').html('¥'+result['amount']);
}
...
function checkOrderForm(frm)
{
  ...
  frm.find('input').each(function(index){
      if($(this).attr('name') == 'shipping' && $(this).attr('checked') == 'checked')
       {
	      shippingSelected = true;
	   }
    
	  if($(this).attr('name') == 'payment' && $(this).attr('checked') == 'checked')
       {
  ...
  {
    var integral = $("#ECS_INTEGRAL").html();
