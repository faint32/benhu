...
$(function(){
	...
	$(".minicart_box").mouseleave(function(){
	 ...
	 /******小购物车：套餐搭配处，总金额的计算******/
	  var discount_rate = $('.sc_main_goods').attr('data-discount_rate')
    $(".index_sc_taocan").each(function(){
	    var perPackageTotal = 0.0
	
	   $(this).find('.price').each(function(){
	      perPackageTotal +=  parseFloat($(this).attr('data-shop-price'))
	   });
	   
	   	var perNum = parseInt($(this).find('.per_number').attr('data-per-number'))
		perPackageTotal *= perNum*discount_rate
	   $(this).find('.per_amount').html('¥' + perPackageTotal.toFixed(2))
	});
})
