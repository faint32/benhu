...
class integrate
{
     ...
     * @return int
     */
    function add_user($username, $password, $email = 0, $gender = -1, $bday = 0, $reg_date=0, $md5password='')
    {
        /* 将用户添加到整合方 */
        if ($this->check_user($username) > 0)
        {
            $this->error = ERR_USERNAME_EXISTS;
           echo "location:includes\modules\integrates\integrate.php add_user 将用户添加到整合方，出现错误<br>";
            return false;
        }
