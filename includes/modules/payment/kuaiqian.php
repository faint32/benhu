...
class kuaiqian
{
   ...
   /**
     ...
     */
   function get_code($order, $payment, $payType='00', $bankId='')
   {
       ...
       $product_name       = '笨虎之家商品订单';
       $product_num        = '';
       $product_id         = '';
       $product_desc       = '';
       $ext1               = $order['log_id'];
       $ext2               = '';
       $pay_type           = $payType;                                                 //支付方式 不可空
       $bank_id            = $bankId;
       $redo_flag          = '0';
       $pid                = '';
//var_dump($order);
//die;
        ...
        $def_url  = '<div style="position: absolute; opacity: 0; filter:Alpha(opacity=0);"><form name="kqPay" style="text-align:center;" method="post" action="https://www.99bill.com/gateway/recvMerchantInfoAction.htm" target="_blank">';
		...
		$def_url .= "<input type='submit' id='pay_btn' name='submit' value='" . $GLOBALS['_LANG']['pay_button'] . "' />";
		$def_url .= "</form></div>";
        return $def_url;
    }
    ...
    function respond()
    {
       ...
       if(!is_numeric($ext1) || !($ext1))
		 {	
		   $sql = "select order_id from ".$GLOBALS['ecs']->table('order_info')." where order_sn='".$order_id."'";
		   $orderId = $GLOBALS['db']->getOne($sql);
			$sql = "select log_id from ".$GLOBALS['ecs']->table('pay_log')." where order_id='".$orderId."'";
			$ext1 = $GLOBALS['db']->getOne($sql);
		 }
