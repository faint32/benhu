...
{
    /* 计算订单各种费用之和的语句 */
    $total_fee = " (goods_amount - discount + tax + shipping_fee + insure_fee  + pack_fee + card_fee) AS total_fee ";
...
function order_goods($order_id)
{
    ...
    $sql = "SELECT rec_id, goods_id, packageBuyId, packageBuyDiscount, goods_name, goods_sn, market_price, goods_number, " .
            "goods_price, goods_attr, is_real, parent_id, is_gift, " .
            "goods_price * goods_number AS subtotal, extension_code " .
            "FROM " . $GLOBALS['ecs']->table('order_goods') .
            " WHERE order_id = '$order_id' order by packageBuyId asc";
...
function order_fee($order, $goods, $consignee)
{
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            ...
            $shipping_count = 1;//$GLOBALS['db']->getOne($sql);
    ...
    // 购物车中的商品能享受红包支付的总额
    $coupon_amount = 0;//compute_discount_amount();
    // 红包和积分最多能支付的金额为商品总额
    ...
    else
    {
        ...
        $total['amount'] -= $total['coupons']; // 还需要支付的订单金额
    }
    ...
    if ($order['extension_code'] == 'exchange_goods')
    {
		...
		$sql = 'SELECT SUM(c.needed_money * c.goods_number) '.
               'FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c,' . $GLOBALS['ecs']->table('exchange_goods') . 'AS eg '.
               "WHERE c.goods_id = eg.goods_id AND c.session_id= '" . SESS_ID . "' " .
               "  AND c.rec_type = '" . CART_EXCHANGE_GOODS . "' " .
               '  AND c.is_gift = 0 AND c.goods_id > 0 ' .
	...
	else if($order['extension_code'] == 'vip_goods')
	{ 
	     $sql = 'SELECT (vmp.user_integral * c.goods_number) '.
               'FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c,' . $GLOBALS['ecs']->table('vip_member_price') . 'AS vmp '.
               "WHERE c.goods_id = vmp.goods_id AND c.session_id= '" . SESS_ID . "' " .
               "  AND c.rec_type = '" . CART_EXCHANGE_GOODS . "' " .
               '  AND c.is_gift = 0 AND c.goods_id > 0 AND c.needed_money = vmp.user_price ' .
               'GROUP BY vmp.goods_id';
        $exchange_integral = $GLOBALS['db']->getOne($sql);
        $total['exchange_integral'] = $exchange_integral;
		$sql = 'SELECT (c.needed_money * c.goods_number) '.
               'FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c,' . $GLOBALS['ecs']->table('vip_member_price') . 'AS vmp '.
               "WHERE c.goods_id = vmp.goods_id AND c.session_id= '" . SESS_ID . "' " .
               "  AND c.rec_type = '" . CART_EXCHANGE_GOODS . "' " .
               '  AND c.is_gift = 0 AND c.goods_id > 0 AND c.needed_money = vmp.user_price ' .
...
function GetRequest() 
{
   ...
   if($pos)
   {
     ...
     $getParas = explode('&', $url);

     for ($i=0; $i <count($getParas) ; $i++) { 
        $para = explode('=', $getParas[$i]);
        $theRequest[$para[0]] =  $para[1];
     }
   }
 ...
 * @param   array   $goods_list 类型: 选中的商品的商品id数组(新增参数)
 * @return  array   购物车商品数组
 */
function cart_goods($type = CART_GENERAL_GOODS , $goods_list = '', $buyType = '')
{
   ...
   {
    $in = db_create_in($goods_list,'c.rec_id')." and ";
	if($buyType == 'buy_now') $in = db_create_in($goods_list,'c.goods_id')." and ";
   }
    $sql = "SELECT c.*,g.goods_thumb " .
         //   "c.goods_price * c.goods_number AS subtotal " .
            "FROM " . $GLOBALS['ecs']->table($cart_table) ." AS c "." LEFT JOIN ".$GLOBALS['ecs']->table('goods')." AS g ON g.goods_id=c.goods_id  WHERE $in  session_id = '" . SESS_ID . "' " ."AND rec_type = '$type' order by packageBuy_id ASC";//新增加了商品图片guan
    ...
    foreach ($arr as $key => $value)
    {
	     ...
	     $activityPrice = getGoodActivityPrice( $value['goods_id']);
	      $value['goods_price'] = ($activityPrice > 0 ) ? $activityPrice : $value['goods_price'];
		 /* echo $value['goods_price'];
		echo gettype($value['goods_price'])."<br/>";
		echo gettype( intval($value['goods_number']) )."<br/>";
		 */
		 //echo $value['goods_price'] * intval($value['goods_number']) ;
		 $arr[$key]['goods_price'] = $value['goods_price'];
        $arr[$key]['formated_market_price'] = price_format($value['market_price'], false);
        $arr[$key]['formated_goods_price']  = price_format($value['goods_price'], false);
        $arr[$key]['formated_subtotal']     = price_format( $value['goods_price'] * intval($value['goods_number']) , false);
        $arr[$key]['url']  = build_uri('goods', array('gid' => $arr[$key]['goods_id']), $arr[$key]['goods_name']);   
        if ($value['extension_code'] == 'package_buy')
        {
            $arr[$key]['package_goods_list'] = get_package_goods($value['goods_id']);
        }
		      if($key == 0 && $arr[$key]['packageBuy_id'] !=0)
			  {
			    $arr[$key]['packageBuy']= 1;
			  }
		      elseif($key > 0 && $arr[$key]['packageBuy_id'] !=0 && $arr[$key]['packageBuy_id'] != $arr[$key-1]['packageBuy_id'] )
				{
				    	$arr[$key]['packageBuy']= 1;
				}
				else if($arr[$key]['packageBuy_id'] !=0 && $arr[$key]['packageBuy_id'] == $arr[$key-1]['packageBuy_id'])
				{
						$arr[$key]['packageBuy']= 3;
						if($arr[$key-1]['packageBuy'] == 3)
						$arr[$key-1]['packageBuy'] == 2;
				}
    }
//var_dump($arr);
    return $arr;
...
function cart_amount($include_gift = true, $type = CART_GENERAL_GOODS)
{
    $cart_table = get_cart_table();
    $sql = "SELECT SUM(goods_price * goods_number) " .
            " FROM " . $GLOBALS['ecs']->table($cart_table) .
 ...
 * @return  boolean
 */
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0, $type='', $packageBuy_id = '')
{
  switch($type)
  {
    ...
    if ($parent > 0)
    {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table($cart_table) .
    ...
    foreach ($basic_list as $parent_id => $fitting_price)
    {
        ...
        /* 添加 */
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table($cart_table), $parent, 'INSERT');
    ...
    {
        /* 检查该商品是否已经存在在购物车中 */
        $sql = "SELECT goods_number FROM " .$GLOBALS['ecs']->table($cart_table).
                " WHERE session_id = '" .SESS_ID. "' AND goods_id = '$goods_id' ".
                " AND parent_id = 0 AND goods_attr = '" .get_goods_attr_info($spec). "' " .
                " AND extension_code <> 'package_buy' " .
                " AND rec_type = 'CART_GENERAL_GOODS' AND group_id='' AND packageBuy_id = '0'";//by mike add

        $row = $GLOBALS['db']->getRow($sql);

        if($row && $packageBuy_id == '') //如果购物车已经有此物品，则更新
        {
            ...
            {
                $goods_price = get_final_price($goods_id, $num, true, $spec);
                $sql = "UPDATE " . $GLOBALS['ecs']->table($cart_table) . " SET goods_number = '$num'" .
    ...
    /* 把赠品删除 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table($cart_table) . " WHERE session_id = '" . SESS_ID . "' AND is_gift <> 0";
    $GLOBALS['db']->query($sql);
 ...
 * @param   int     $type   类型：默认普通商品
 */
function clear_cart($type = CART_GENERAL_GOODS, $buyType = '')
{
 switch($buyType)
  {
    case 'buy_now': $cart_table = 'cart_buy_now'; break;
    ...
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table($cart_table) .
...
/**
 * 设置购物券为已使用
 * @param   int     $coupons_id   购物券id
 * @param   int     $order_id   订单id
 * @return  bool
 */
function use_coupons($coupons_id, $order_id)
{
    $sql = "UPDATE " . $GLOBALS['ecs']->table('coupons_sent') .
            " SET order_id = '$order_id', is_used = 1, used_time = '" . gmtime() . "' " .
            "WHERE sent_coupon_id = '$coupons_id' LIMIT 1";

    return  $GLOBALS['db']->query($sql);
}
/**
 * 取得红包信息
 * @param   int     $couponsId   购物券id
 * @param   array   购物券信息
 */
function coupons_info($couponsId)
{
    $sql = "SELECT * " .
            "FROM " . $GLOBALS['ecs']->table('coupons_sent') .
            "WHERE sent_coupon_id = '$couponsId' ";
    return $GLOBALS['db']->getRow($sql);
}
 ...
 switch($type)
  {
    ...
    $sql = "SELECT *, IF(parent_id, parent_id, goods_id) AS pid " .
            ...
            " ORDER BY packageBuy_id asc, pid, parent_id";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
	   if( ($row['packageBuy_id'] == $packageId) && ($row['packageBuy_id'] !=0) ) //最后一件套餐购商品
	   {
	       $row['packageBuy'] = 3;
		   
		   if($goodsListKey <= 0 ) die('error: array index is less than 0. location:lib_order.php ');
		   if($goods_list[$goodsListKey - 1]['packageBuy'] == 3)  //去掉前一个商品的最后一个标记
		   {
		      $goods_list[$goodsListKey - 1]['packageBuy'] = 2;  
		   }
	   }
	   else if($row['packageBuy_id'] != 0) //第一件套餐购商品
	   {
	     $row['packageBuy'] = 1;
	   }
	   else                                //普通商品
 ...
 * @return  array
 */
function get_consignee($user_id, $set_moren)
{
        ...
        if ($user_id > 0)
        {
			$sql="SELECT is_first FROM " . $GLOBALS['ecs']->table('user_address') . " WHERE user_id='$user_id'";
            $a=$GLOBALS['db']->getAll($sql);
			
             $isfirst = "";
            if($a){ //guan 此处是为了判断收货地址列表中有几个地址，有没有设置为默认地址，
				$tflag = false;
				foreach($a as $row){
					if($row['is_first'] == 1)
					  $tflag = true;	
				}
				if($tflag) $isfirst= " AND is_first=1";
				else $isfirst= " AND is_first=0";
			}

            /* 取默认地址 */
            $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_address') . " WHERE user_id='$user_id' $isfirst";
		 ...
		 if($arr['district']){ 
            ...
            $city=$arr['city'];
            $dis=$arr['district'];
            $add=$GLOBALS['db']->getAll("SELECT region_name FROM " .$GLOBALS['ecs']->table('region'). " WHERE region_id in ($pro,$city,$dis)");
            $a=array(2,25,32,27,33,34,35,28,29,19);//直辖市：北京2，上海25，重庆32，天津27，港澳台33,34,35，西藏28，新疆29，内蒙古19
            $b=array(2,25,32,27);
            if(in_array($pro,$a)){
                if(in_array($pro,$b)){
                    $arr['address1']=$add[1]['region_name']."市".$add[2]['region_name'];
                }  else {
                    $arr['address1']=$add[0]['region_name']."省".$add[1]['region_name']."市".$add[2]['region_name'];
                }
            }  else {
                $arr['address1']=$add[0]['region_name']."省".$add[1]['region_name']."市".$add[2]['region_name'];
            }
...
function get_cart_table()
{
    ...
    if(!empty($getPara['buyType']))
    {
        $buyType = $getPara['buyType'];
    }
...
/*取得最近30天：购买的人数和交易中的人数chen-0926*/
function recently_buy($goods_id)
{
    ...
    $trading_sql = "SELECT COUNT(oi.order_id) " .
            " FROM " . $GLOBALS['ecs']->table('order_info') . " as oi ".
			" join " . $GLOBALS['ecs']->table('order_goods') . " as og ".
			" on oi.order_id = og.order_id ".
            "WHERE  oi.add_time >=" . $mothBef .
			" and ~(order_status = 2 and shipping_status = 0 and pay_status = 0) and 
			og.goods_id = $goods_id";
		
	$where_saled = order_query_sql('finished');	
			$traded_sql = "SELECT COUNT(oi.order_id) " .
            " FROM " . $GLOBALS['ecs']->table('order_info') . " as oi ".
			" join " . $GLOBALS['ecs']->table('order_goods') . " as og ".
			" on oi.order_id = og.order_id ".
            "WHERE  oi.add_time >=" . $mothBef .
			" $where_saled and og.goods_id = $goods_id ".$all_order_list->complete_trade;
	$recently_buy=array();
...
function getsales_history($goods_id){
 ...
 f.pay_time,g.goods_attr FROM '. $GLOBALS['ecs']->table('order_goods') .' as g,'. 
 $GLOBALS['ecs']->table('users').' as u, '.
 $GLOBALS['ecs']->table('order_info') .' as f where u.user_id = f.user_id and g.order_id = f.order_id '.$where_saled.'and g.goods_id='.$goods_id.' order by f.pay_time desc';
...
function getPackageBuyDiscount()
{
    $packageBuyDiscount = floatval($GLOBALS['_CFG']['package_buy']);
	$packageBuyDiscount = ($packageBuyDiscount > 1.0 || $packageBuyDiscount < 0.0) ? 1.0 : $packageBuyDiscount; 
