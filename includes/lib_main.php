...
function update_user_info()
{
    ...
    $GLOBALS['db']->query($sql);
     /* 更新用户购物车session_id guan 退出不清空购物车*/
...
function get_user_info($id=0)
{
    ...
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_id, u.email, u.user_name, u.nickname, u.user_money, u.pay_points'.
            ' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    $ur_here    = '<a href=".">' . $GLOBALS['_LANG']['home'] . '</a>&nbsp;';//0828-guan
    /* 根据文件名分别处理中间的部分 */
    if ($filename != 'index')
    {
        /* 处理有分类的 */
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand')))
        {
            ...
            if (!empty($cat_arr))
            {
                ...
                foreach ($cat_arr AS $val)
                {
                    ...
                    $ur_here   .= '&gt;&nbsp;<a href="' . build_uri($type, $args, $val['cat_name']) . '">' .htmlspecialchars($val['cat_name']) . '</a>&nbsp;';//0828-guan
                }
            }
        }
        /* 处理无分类的 */
    ...
    if (!empty($str))
    {
        ...
        //$ur_here    .= '<span><a href="javascript:void(0);">'.$str.'</a></span>';
 ...
 * @return  array
 */
function get_all_cats($cat)
{
    $arr1 = $GLOBALS['db']->GetOne('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['ecs']->table('category').'WHERE cat_id ='.$cat);
...
function get_shop_help()
{
    ...
    {
        $arr[$row['cat_id']]['cat_id']                       = $row['cat_id'];//0828-guan
    ...
    $arr['package_buy'] = $GLOBALS['_CFG']['package_buy'];
 ...
 * @return  array       $pager
 */
function get_pager($url, $param, $record_count, $page = 1, $size = 10, $index='page=', $contrl='')
{
    $size = intval($size);
    if ($size < 1)
    {
    ...
    if ($pager['styleid'] == 0)
    {
        $pager['page_first']   = $url . $param_url . $index.'1'.$contrl;
        $pager['page_prev']    = $url . $param_url . $index . $page_prev.$contrl;
        $pager['page_next']    = $url . $param_url . $index . $page_next.$contrl;
        $pager['page_last']    = $url . $param_url . $index . $page_count.$contrl;
        $pager['array']  = array();
        for ($i = 1; $i <= $page_count; $i++)
        {
            $pager['array'][$i] = $i;
    ...
    else
    {
        ...
        $url_format = $url . $param_url . $index;
        $pager['page_first'] = ($page - $_offset > 1 && $_pagenum < $page_count) ? $url_format . 1 .$contrl : '';
        $pager['page_prev']  = ($page > 1) ? $url_format . $page_prev .$contrl : '';
        $pager['page_next']  = ($page < $page_count) ? $url_format . $page_next .$contrl: '';
        $pager['page_last']  = ($_to < $page_count) ? $url_format . $page_count.$contrl : '';
        $pager['page_kbd']  = ($_pagenum < $page_count) ? true : false;
        $pager['page_number'] = array();
        for ($i=$_from;$i<=$_to;++$i)
        {
            $pager['page_number'][$i] = $url_format . $i .$contrl;
        }
    }
    $pager['search'] = $param;
...
function assign_comment($id, $type, $page = 1)
{
    ...
    $sql = 'SELECT c.*,u.rank_points,u.personal_pic FROM ' . $GLOBALS['ecs']->table('comment').' AS c LEFT JOIN '.$GLOBALS['ecs']->table('users')." AS u ON c.user_id=u.user_id WHERE id_value = '$id' AND comment_type = '$type' AND c.status = 1 AND c.parent_id = 0".
    ...
    /*获取会员等级数据*/
    $sql  = 'SELECT min_points,max_points FROM '.$GLOBALS['ecs']->table('user_rank').' WHERE rank_name LIKE "VIP%" ORDER BY min_points ASC';
    ...
    {
      if($row['order_id']){
        $sql = "select thumb from ".$GLOBALS['ecs']->table('shaidan')." where order_id=".$row['order_id']." and goods_id=".$row['id_value'];
       $thumbs = $GLOBALS['db']->getAll($sql);  
      }
        $arr[$row['comment_id']]['thumbs'] =empty($row['order_id'])?'': $thumbs;
        $ids .= $ids ? ",$row[comment_id]" : $row['comment_id'];
        $arr[$row['comment_id']]['id']       = $row['comment_id'];
        $arr[$row['comment_id']]['email']    = $row['email'];
         $arr[$row['comment_id']]['username'] =  ($row['is_anonymity'] == 0 )?  $row['user_name']:'';
        $arr[$row['comment_id']]['personal_pic'] = $row['personal_pic'];
         
        if(empty($row['rank_points']) || $row['user_id'] == 0)
        {
             $arr[$row['comment_id']]['rank_portrait'] = 'style/images/vip_s0.png'; 
        }
        else
        {
            foreach($rank_res as $key=>$value)
            {
                ...
                {
                     $arr[$row['comment_id']]['rank_portrait'] = 'style/images/vip_s'.$key.'.png';
                }
            }
        }
    ...
    /* 分页样式 */
    $pager = getPager(array('currPage' => $page, 'size' => $size, 'count' => $count, 'page_count' => $page_count, 'id' => $id, 'type' => $type));
...
function getMyPage($para)
{
    $pager['page']         = $para['currPage'];
    $pager['size']         = $para['size'];
    $pager['record_count'] = $para['count'];
    $pager['page_count']   = $para['page_count'];

    $jsFuncPara = '';
    foreach ($para['ext'] as  $value) 
    {
       $jsFuncPara .= ",'".$value."'";
    }

    $js_func = $para['js_func'];  //点击页码按钮，所调用的js函数
    $pager['page_first']   = "javascript:$js_func(1 $jsFuncPara)";
    $pager['page_prev']    = ($pager['page'] > 1) ? "javascript:$js_func(" .($pager['page']-1). " $jsFuncPara)" : 'javascript:;';
    $pager['page_next']    = ($pager['page'] < $pager['page_count']) ? "javascript:$js_func(" .($pager['page'] + 1) . " $jsFuncPara)" : 'javascript:;';
    $pager['page_last']    = $pager['page'] < $pager['page_count'] ? "javascript:$js_func(" .$pager['page_count']. " $jsFuncPara)"  : 'javascript:;';
    
        $pageListFst = ($para['currPage'] - 5 >= 1) ? ($para['currPage'] - 5) : 1;
        $pageListLst = ($para['currPage'] + 4 <= $para['page_count']) ? ($para['currPage'] + 4) : $para['page_count']; 
     if($pager['page_count'] > 1)
     {   
          for($i = $pageListFst; $i <= $pageListLst; $i++)
          {
            $tmp = "javascript:$js_func(" . $i . " $jsFuncPara)";
           
           if($i == $para['currPage'])
            $tmp = 'javascript:;';
            
            $pager['page_list'][$i] = $tmp;
          }
     }
    return $pager; 
}

function getBoughtPage($para)
{
      $id = $para['id'];
    ...
    $pager['record_count'] = $para['count'];
    $pager['page_count']   = $para['page_count'];
    $pager['page_first']   = "javascript:gotoBuyPage(1,$id)";
    $pager['page_prev']    = ($pager['page'] > 1) ? "javascript:gotoBuyPage(" .($pager['page']-1). ",$id)" : 'javascript:;';
    $pager['page_next']    = ($pager['page'] < $pager['page_count']) ? 'javascript:gotoBuyPage(' .($pager['page'] + 1) . ",$id)" : 'javascript:;';
    $pager['page_last']    = $pager['page'] < $pager['page_count'] ? 'javascript:gotoBuyPage(' .$pager['page_count']. ",$id)"  : 'javascript:;';
     ...
     {   
          for($i = $pageListFst; $i <= $pageListLst; $i++)
          {
            $tmp = 'javascript:gotoBuyPage(' . $i . ",$id)";
...
function getPager($para)
{
    ...
    $pager['page_prev']    = ($pager['page'] > 1) ? "javascript:gotoPage(" .($pager['page']-1). ",$id,$type)" : 'javascript:;';
    $pager['page_next']    = ($pager['page'] < $pager['page_count']) ? 'javascript:gotoPage(' .($pager['page'] + 1) . ",$id,$type)" : 'javascript:;';
    $pager['page_last']    = $pager['page'] < $pager['page_count'] ? 'javascript:gotoPage(' .$pager['page_count']. ",$id,$type)"  : 'javascript:;';
     ...
     {   
          for($i = $pageListFst; $i <= $pageListLst; $i++)
          {
            $tmp = 'javascript:gotoPage(' . $i . ",$id,$type)";
    ...
    */
    ...
    if ($GLOBALS['db']->getOne($sql))
    {
        ...
        $sql = 'SELECT a.cat_id, a.cat_name, a.sort_order AS parent_order, a.cat_id, ' .
                ...
                "WHERE a.parent_id = '$parent_id' AND a.cat_type in(1,2) ORDER BY parent_order ASC, a.cat_id ASC, child_order ASC";
    }
    else
    {
        /* 获取当前分类及其父分类 */
    ...
    foreach ($res AS $row)
    {
        ...
        $sql1='SELECT parent_id FROM '. $GLOBALS['ecs']->table('article_cat') .' WHERE cat_id ='.$id;
       // echo "<br>-----".$id."====".$GLOBALS['db']->getOne($sql1)."=====++++".$row['cat_id']."<br>";
        if ($GLOBALS['db']->getOne($sql1) == $row['cat_id'])
         $cat_arr[$row['cat_id']]['id1']=1;
        else
        $cat_arr[$row['cat_id']]['id1']=0;
...
function get_library_number($library, $template = null)
{
    ...
    else
    {
        ...
        $num = isset($static_page_libs[$template][$lib]) ? $static_page_libs[$template][$lib] :  8;
    }
