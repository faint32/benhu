 ...
 * return $res
 */
function get_level_category1($cat_id)
{
	$sql = 'SELECT parent_id FROM '.$GLOBALS['ecs']->table('category').' WHERE cat_id = '.$cat_id;
	$parent_id = $GLOBALS['db']->getOne($sql);
	$sql = 'SELECT cat_id,cat_name FROM '.$GLOBALS['ecs']->table('category').' WHERE parent_id = '.$parent_id;
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        foreach ($result AS $idx => $row)
        {
		   ...
		   // require_once(ROOT_PATH . 'includes/lib_order_status.php');
		   require_once(ROOT_PATH . 'includes/lib_order.php');
			$sql = 'SELECT sum(goods_number) AS goods_number FROM '.$GLOBALS['ecs']->table('order_goods').' AS og '.
			' JOIN '.$GLOBALS['ecs']->table('order_info').' AS oi ON og.order_id = oi.order_id '.
			' WHERE goods_id ='.$row['goods_id'].' '.order_query_sql( 'finished','oi.').' GROUP BY goods_id';
...
//echo $sql;
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		//*
		/*
		$sql = "select count(goods_id) from ".$GLOBALS['ecs']->table('order_goods')." where goods_id=".$row['goods_id'];
		//$sql = "select count(goods_id) from ".$GLOBALS['ecs']->table('delivery_goods')." where goods_id=".$row['goods_id'];
        $goods[$idx]['selledNum']         = $GLOBALS['db']->getOne($sql);
		*/
		//*/
	//	$goods[$idx]['selledNum']         = $row['selledNum'];
		$goods[$idx]['name']         = $row['goods_name'];
        $goods[$idx]['brief']        = $row['goods_brief'];
        $goods[$idx]['brand_name']   = $row['brand_name'];
        $goods[$idx]['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
...
function get_category_hotSale_goods($cat=' 1 ')
{
/*$sql = "select sum(1) as selledNum, g.goods_id, g.goods_name as goods_name ,goods_thumb,shop_price,promote_price,promote_start_date,promote_end_date,goods_name_style ".
        " from ".$GLOBALS['ecs']->table('goods')." as g join ".$GLOBALS['ecs']->table('delivery_goods')." as dg".
		" on g.goods_id = dg.goods_id where $cat group by g.goods_id order by selledNum desc";
*/
//$where_saled = "  AND oi.order_status  IN ('1','5')  AND oi.shipping_status  IN ('1','2')  AND oi.pay_status  IN ('2','1')";// oi.order_status = 5 and oi.shipping_status = 2 and oi.pay_status = 2 and ';
include_once(ROOT_PATH . 'includes/lib_order.php');

$where_saled = order_query_sql($type = 'finished');
$sql = "select sum(1) as selledNum, g.goods_id, g.goods_name as goods_name ,goods_thumb,shop_price,promote_price,promote_start_date,promote_end_date,goods_name_style ".
        " from ".$GLOBALS['ecs']->table('goods')." as g join ".$GLOBALS['ecs']->table('order_goods')." as og on g.goods_id = og.goods_id".
		" join ".$GLOBALS['ecs']->table('order_info')." as oi".
		" on og.order_id = oi.order_id where  $cat $where_saled group by g.goods_id order by selledNum desc";
		//echo $sql;
		 $goods = $GLOBALS['db']->getAll($sql);
		 if(count($goods) <= 0)
		 {
		  $sql = "select 0 as selledNum, goods_id, goods_name as goods_name ,goods_thumb,shop_price,promote_price,promote_start_date,promote_end_date,goods_name_style ".
        " from ".$GLOBALS['ecs']->table('goods')." as g ".
		" where $cat order by rand() limit 6 "; 
		$goods = $GLOBALS['db']->getAll($sql);
		 }
		 
		  foreach($goods as $goodKey => $goodVal)
			 {
				$goods[$goodKey]['url'] = build_uri('goods', array('gid' => $goodVal['goods_id']), $goodVal['goods_name']);
				$goods[$goodKey]['thumb']  = $goodVal['goods_thumb'];
				$goods[$goodKey]['shop_price']  = price_format($goodVal['shop_price']);
				
				$goodVal['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
										   sub_str($goodVal['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $goodVal['goods_name'];
				$goods[$goodKey]['short_style_name'] = add_style($goodVal['short_name'], $goodVal['goods_name_style']);
				
						 if ($goods[$goodKey]['promote_price'] > 0)
					{
						$promote_price = bargain_price(	$goods[$goodKey]['promote_price'], 	$goods[$goodKey]['promote_start_date'], $goods[$goodKey]['promote_end_date']);
						$goods[$goodKey]['promote_price'] = $promote_price > 0 ? price_format($promote_price) : '';
					}
					else
					{
						$goods[$goodKey]['promote_price'] = '';
					}
			 }
...
function getGoodActivityPrice($goods_id)
{
	$gmtime = gmtime();
	$sql = 'SELECT act_id, act_range, act_range_ext, act_type, act_type_ext, act_name, start_time, end_time FROM ' .
			$GLOBALS['ecs']->table('favourable_activity') . 
			" WHERE start_time <= '$gmtime' AND end_time >= '$gmtime'";
	$activitys = $GLOBALS['db']->getAll($sql);
		$sql = "SELECT cat_id, brand_id, shop_price, market_price FROM " . $GLOBALS['ecs']->table('goods') .
			   "WHERE goods_id = '$goods_id'";
			 
        $good = $GLOBALS['db']->getRow($sql);
        
		$category_id = $good['cat_id'];
		
        $brand_id = $good['brand_id'];
		
		$activityPrice = $good['market_price'];
		...
		foreach($activitys as $activity)
		{
			if ($activity['act_range'] == FAR_CATEGORY)
			{
				...
				if(isCategoryIDExistInActivitys($category_id, $activity['act_range_ext']))
				{
				
					calcActivityPrice($activityPrice, $activity['act_type'], $activity['act_type_ext']);
					
				    break;
				}
			}
		    elseif ($activity['act_range'] == FAR_BRAND)
            {
			
                if (strpos(',' . $activity['act_range_ext'] . ',', ',' . $brand_id . ',') !== false)
                {
			...
			elseif ($activity['act_range'] == FAR_GOODS)
            {
                if (strpos(',' . $activity['act_range_ext'] . ',', ',' . $goods_id . ',') !== false)
                {
					calcActivityPrice($activityPrice, $activity['act_type'], $activity['act_type_ext']);
                    break;
                }
		...
		$activityPrice = ($activityPrice == $good['market_price']) ? -1 : $activityPrice;
return $activityPrice;
}


function getGoodActivityInfo($goods_id)
{
	$gmtime = gmtime();
	$sql = 'SELECT act_id, act_range, act_range_ext, act_type, act_type_ext, act_name, start_time, end_time FROM ' .
			$GLOBALS['ecs']->table('favourable_activity') . 
	...
	$activitys = $GLOBALS['db']->getAll($sql);
		...
		$activityPrice =  $good['market_price']; $activityName = '';
...
function calcActivityPrice(&$activityPrice, $act_type, $act_type_ext)
{
  switch($act_type)
  {
    case 1:  //现金减免
		$activityPrice -= floatval($act_type_ext); 
		$activityPrice = ($activityPrice <= 0) ? 0 : $activityPrice;
		break;
	case 2: //价格折扣
	    $act_type_ext = floatval($act_type_ext)/100.0;
	    $act_type_ext = ($act_type_ext < 0 || $act_type_ext > 1) ? 1.0 : $act_type_ext;
		$activityPrice *= $act_type_ext;
		break;
  }
}

function isCategoryIDExistInActivitys($category_id, $categorys)
{
	/* 找出分类id的子分类id */
	$id_list = array();
	$raw_id_list = explode(',', $categorys);
...
function favourable_info2($act_id)
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('favourable_activity') .
            " WHERE act_id = '$act_id' AND activity_type = 2";
    $row = $GLOBALS['db']->getRow($sql);
    if (!empty($row))
    {
        $row['start_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['start_time']);
		...
		if ($GLOBALS['db']->getOne($sql))
		{
			...
			foreach ($res AS $row)
			{
				...
				if($two_goods == 1)
				{
					...
					"ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
					'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND '.
					'g.is_delete = 0 AND (' . $children . 'OR ' . get_extension_goods($children) . ')  ORDER BY g.sort_order, g.goods_id DESC';
...
function get_child_tree_goods($tree_id = 0)
{
    ...
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        $child_sql = 'SELECT cat_id, cat_name, parent_id, is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
                "WHERE parent_id = '$tree_id' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC LIMIT 7";
        $res = $GLOBALS['db']->getAll($child_sql);
        foreach ($res AS $row)
        {
            if ($row['is_show'])
