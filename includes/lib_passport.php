...
function register($username, $password)
{
    ...
    // echo "和管理员重名<br>";
     ...
     $empEmail='';
    if (!$GLOBALS['user']->add_user($username, $password,$empEmail))
    {
   //   echo "add_user判断进来<br>";
...
/**
 *  用户进行密码找回操作时，发送一条验证短信
 *
 * @access  public
 * @param   string  $uid          用户ID
 * @param   string  $user_name    用户手机号
 * @param   string  $code         key
 * @return  boolen  $result;
 */
function send_verify_shortMessage($uid, $user_name, $code) 
{
   if (empty($uid) || empty($user_name) || empty($code))
    {
        ecs_header("Location: user.php?act=get_password\n");  
        exit;
    }
...
function send_pwd_email($uid, $user_name, $email, $code)
{
    ...
    $reset_email = $GLOBALS['ecs']->url() . 'user.php?act=get_password&uid=' . $uid . '&code=' . $code.'&cookie_value='.rand(0, 99999);
 ...
 * @access  public
 * @param   int     $user_id        用户ID
 * @param   string    $type         验证类型：验证邮箱，解除邮箱
 * @return boolen
 */
function send_regiter_hash ($user_id, $type = "register")
{
    /* 设置验证邮件模板所需要的内容信息 */
    $template    = get_mail_template('register_validate');
	if($type == "disbind") $template = get_mail_template('disbind_email');
	else if($type == "emailUserValid") $template = get_mail_template('emailUser_register_email');
    $hash = register_hash('encode', $user_id);
    $validate_email = $GLOBALS['ecs']->url() . 'user.php?act=validate_email&hash=' . $hash.'&type='.$type;

    $sql = "SELECT nickname, email,user_name FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
   if($type=="emailUserValid") $row['email'] = $row['user_name'];

    $GLOBALS['smarty']->assign('user_name',         $row['nickname']);
 ...
 /* 发送激活验证邮件 */
    if (send_mail($row['email'], $row['email'], $template['template_subject'], $content, $template['is_html']))
    {
...
/**
 * 接受用户的邮箱地址，返回其邮箱对应的域名
 * @param   string      $mail_addr 邮箱地址
 * @return  对应域名
 */
function get_mail_serverAddr($mail_addr)
{
    $ofnUsdMl=array(
    ...
    '@56'=>'http://www.56.com/home.html'
    );
    $rule = '/(@qq)|(@gmail)|(@163)|(@139)|(@sina)|(@hotmail)|(@live)|(@outlook)|(@yahoo)|(@sohu)|(@21cn)|(@tom)|(@eyou)|(@56)/';
      preg_match($rule,$mail_addr,$result); 
      if($result[0]=='@live' || $result[0]=='@outlook') 
          $result[0]= '@hotmail';
            
      return $ofnUsdMl[$result[0]];
}
?>