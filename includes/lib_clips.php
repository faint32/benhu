 ...
 */
/**
 *8-22          新增  get_comment_list,get_complain_list,return_goods_history函数
 *              --功能：获取用户评论，     投诉/建议   ,  退货记录
 */
 /**monthly_order_list
 *8-25          新增  monthly_order_list 函数
 *              --功能：获取用户近一个月的订单(已付过款的)
 */
if (!defined('IN_ECS'))
{
    die('Hacking attempt');
...
/**
 * 获取用户中心默认页面所需的数据  chen-0911 增加chat_validated，用于手机,邮箱的验证
 *
 * @access  public
 * @param   int         $user_id            用户ID
 *
...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points,rank_points, user_money, credit_line, last_login, is_validated,chat_validated FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
	...
	$info['chat_validated'] =  $row['chat_validated'];  
...
{
    $sql = "SELECT c.*, g.goods_name AS cmt_name,g.goods_id,g.goods_thumb, r.content AS reply_content, r.add_time AS reply_time ".
           ...
           " WHERE c.user_id='$user_id' order by add_time desc";
 ...
 * @return  array
 */
function get_complain_list($user_id, $page_size, $start)
{

    $sql = "select * from ".$GLOBALS['ecs']->table('complain')." where is_del=0 and user_id=$user_id";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['complain_time'] = local_date('Y-m-d', $row['time']);
    ...
    if ($to_replay)
    {
        $sql = "SELECT complain_id,content from " . $GLOBALS['ecs']->table('reply_complain') . " WHERE " . db_create_in($to_replay, 'complain_id');
        $arr = $GLOBALS['db']->getAll($sql);
        $to_cmt_name = array();
        foreach ($arr as $row)
        {
            $to_cmt_name[$row['complain_id']] = '<strong>客服已回复：</strong>'.$row['content'];
        }
        foreach ($complains as $key=>$row)
        {
            if ($row['state'] == 2)
            {
                $complains[$key]['state'] = isset($to_cmt_name[$row['id']]) ? $to_cmt_name[$row['id']] : '';
            }
        }
    }
    return $complains;
 ...
 * @return  array
 */
function return_goods_history($user_id, $page_size, $start)
{
	$sql = "select b.back_id,b.order_id,b.return_time,b.return_money,o.goods_amount   from ".$GLOBALS['ecs']->table('back_order')." as b".
              " left join ".$GLOBALS['ecs']->table('order_info')." as o ".
			  " on b.order_id=o.order_id ".
	          " where b.is_cancled=0 and b.user_id=$user_id"; 
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
	   $goods_sql = "select goods_id,goods_name from ".$GLOBALS['ecs']->table('order_goods')." where order_id = ".$row['order_id'];
	  $goods_res= $GLOBALS['db']->getAll($goods_sql);
	  
	  $reply_sql = "select reply_time,action_type from ".$GLOBALS['ecs']->table('back_order_reply')." where back_id = ".$row['back_id']." limit 1";
	  $reply_res= $GLOBALS['db']->getAll($reply_sql);
	  
		$row['goods']=$goods_res;
        $row['reply']=$reply_res[0];
		if($row['reply'])
		{
		   $row['reply']['reply_time']= local_date('Y-m-d', $row['reply']['reply_time']);
		}
	
	   
       $row['return_time']= local_date('Y-m-d', $row['return_time']);
	   $return_goods[] = $row;
    }
 ...
 * @return  array
 */
function monthly_order_list($user_id)
{
      $month_ago = gmtime()-60*60*24*30;
     // echo $month_ago;
	  $order_list_sql=" select order_id,order_sn from ".$GLOBALS['ecs']->table('order_info')." where user_id=$user_id and pay_time >$month_ago ";
	 ...
	 // var_dump( $order_list);
	   ...
	   {
		  $goods_sql = "select goods_name from ".$GLOBALS['ecs']->table('order_goods')." where order_id = ".$value['order_id'];
		  $goods_res= $GLOBALS['db']->getAll($goods_sql);
		  $value['goods']=$goods_res;
		  $monthly_order[]=$value;
	   }
...
function get_all_orders($user_id, $num = 10, $start = 0,$where='',$comment = 0, $all = 0)
{
    ...
    $sql = "SELECT order_id, order_sn, discount, order_status, shipping_status, pay_status,shipping_fee, add_time, integral, tax, " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
	   $goods_sql = "select goods_id,goods_name,goods_attr,goods_number,send_number,goods_price,packageBuyId,packageBuyDiscount from ".$GLOBALS['ecs']->table('order_goods').
	   " where order_id = ".$row['order_id']." order by packageBuyId asc";
	   $goods_res= $GLOBALS['db']->getAll($goods_sql);
	   $row['goods_id_list']='';
	   $row['goods_name_list']='';
	   ...
	   for($i=0;$i<count($goods_res);$i++)
	   {
		  ...
		  else 
		  {
		   ...
		   unset($goods_res[$i]['packageBuyId']);
		    calcPackageBuyVal($i, $goods_res);//$goods_res[$i]['packageBuy'] =
	   ...
	   //$tmp = $row['order_status'];
        ...
        $arr[] = array('order_id'       => $row['order_id'],
					   ...
					   'packageBuyDiscountRate'  => $row['packageBuyDiscountRate'], 
					   'packageBuyDiscountMoney' => price_format($row['packageBuyDiscountMoney']),
					   'discount'       => price_format($row['discount']),
					   ...
					   'goods'          => $row['goods'],
					   'goods_id_list'  => $row['goods_id_list'],
					   'one_goods_id'  => $row['one_goods_id'],
					   'goods_name_list'=> $row['goods_name_list'],
					  'one_goods_name' => $row['one_goods_name'],
					  'tax'        => $row['tax'],
					  'one_goods_thumb'=> $row['goods'][0]['goods_thumb']);
    }

    return $arr;
}
function getOrderOneGoodName($goodsNames)
{
	$loc = strpos( $goodsNames,',');
	if(!$loc) $loc = strlen($goodsNames) ;
	return substr($goodsNames,0,$loc);
}
function getOrderOneGoodId($goodsId)
{
  $loc = strpos( $goodsId,',');
  if(!$loc) $loc = strlen($goodsId) ;
  return substr($goodsId,0,$loc);
}

function getOrderStatus($order_status, $shipping_status, $pay_status, $comment = -1, $order_id = '')
{
   // echo "$order_status  $shipping_status   $pay_status<br/>";
	$status = '';
	if($order_status== 2 && $shipping_status ==0  && $pay_status ==0) 
	{
	  $status ='已取消';
	}
	else if($order_status == 0 && $shipping_status == 0  && $pay_status == 0)
	{
	  $status ='待付款';
	}
	else if($order_status == 1 && $shipping_status == 0  && $pay_status ==0)
	{
	  $status ='已确认';
	}
	else if($order_status == 1 && $shipping_status == 0  && $pay_status == 2)
	{
	  $status ='已付款';
	}
	else if(($order_status == 1 || $order_status == 5) && $shipping_status == 3  && $pay_status == 2)
	{
	  $status ='配货中';
	}
	else if($order_status == 1 && $shipping_status ==0  && $pay_status ==2)
	...
	else if($order_status == 5 && $shipping_status ==1  && $pay_status ==2)
	{
	  $status ='待收货';
	}
	else if($order_status == 5 && $shipping_status ==2  && $pay_status ==2 && $comment == 2)
	{
	...
	else if($order_status ==5 && $shipping_status ==2  && $pay_status ==2 && $comment == 1 )
	{
	  $sql = "select comment_id from ".$GLOBALS['ecs']->table('comment')." where order_id=$order_id";
	  if(!$GLOBALS['db']->getOne($sql))
		return -1;
	...
	else if($order_status ==5 && $shipping_status == 2  && $pay_status == 2 && $comment == 0 )
	{
		$sql = "select comment_id from ".$GLOBALS['ecs']->table('comment')." where order_id=$order_id";
		if($GLOBALS['db']->getOne($sql))
			$status ='交易成功';
	...
	else if($order_status == 4 && $shipping_status == 0  && $pay_status == 0)
	{
	  $status ='退货';
	}
	return $status;
