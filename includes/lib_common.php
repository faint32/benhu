...
function getSpecificTime($num)
{
  $specificTime = getDawnTime() + $num * 60 * 60;
  return $specificTime;
}
...
{
    //$sql = 'SELECT brand_id, brand_name FROM ' . $GLOBALS['ecs']->table('brand') . ' ORDER BY sort_order';原始备份0708
  ...
  if(!empty($goods_id))
    ...
    else
    {
        ...
        foreach ($res as $rows)
        {
            ...
            { 
			// $favourable[$rows['act_id']]['act_name'] = $rows['act_name'];
                $favourable[$rows['act_id']]['url'] = getActivityUrl($rows['act_name']); 
				$favourable[$rows['act_id']]['act_name'] = getActivityName($favourable[$rows['act_id']]['url'] );
                $favourable[$rows['act_id']]['time'] = sprintf($GLOBALS['_LANG']['promotion_time'], local_date('Y-m-d', $rows['start_time']), local_date('Y-m-d', $rows['end_time']));
                $favourable[$rows['act_id']]['sort'] = $rows['start_time'];
                $favourable[$rows['act_id']]['type'] = 'favourable';
            }
...
{
   $activityName =  mb_substr($activityName,0,3*2); 
   switch($activityName)
   {
     case '周年':
	    $url = 'newyear.php';
...
function getActivityName($activityURL)
{
  switch($activityURL)
  {
    case 'newyear.php':
    ...
    switch ($app)
    {
        ...
        case 'search':
            ...
            else
            {
				if(!empty($encode))
				{
					$uri = "search.php?encode=".$keyword;
				}
				else
				{
					$uri = "search.php?keyword=".$keyword;
				}	
            }
            break;
        case 'exchange':
...
function buildUriWithOneParamOrLess($app,  $param = '')
{
 static $rewrite = NULL;
...
function get_personalPic() 
{	

 if($_SESSION['user_id']>0)
  {
   $sql = "select personal_pic from ".$GLOBALS['ecs']->table('users')." where user_id=".$_SESSION['user_id'];
...
function get_express_name($express_type,$express_company)
{
$express_name='';
$express_list = '';
  switch($express_type)
  {
   case 'aicha':
   $express_list = get_aicha_expresses();
   break;
  }
  //var_dump($express_list);
  foreach($express_list as $expressKey => $expressVal)
  {
    if($expressKey == $express_company)
	{
	  $express_name = $expressVal;
	  break;
	}
  }
  return $express_name;
...
function get_express_info($express_type,$express_company,$express_no)
{
switch($express_type)
{
  case 'aicha':  
