...
function insert_cart_flow_content()
{
    ...
    $row = $GLOBALS['db']->GetAll($sql);
        ...
        $arr[$k]['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
                ...
                $arr[$k]['packageBuy_id']= $v['packageBuy_id'];
                $arr[$k]['packageBuy']= 0;
                  if($k == 0 && $arr[$k]['packageBuy_id'] !=0)
                  {
                    $arr[$k]['packageBuy']= 1;
                  }
                elseif($k > 0 && $arr[$k]['packageBuy_id'] !=0 && $arr[$k]['packageBuy_id'] != $arr[$k-1]['packageBuy_id'] )
                {
                        $arr[$k]['packageBuy']= 1;
                }
                else if($arr[$k]['packageBuy_id'] !=0 && $arr[$k]['packageBuy_id'] == $arr[$k-1]['packageBuy_id'])
                {
                        $arr[$k]['packageBuy']= 3;
                        if($arr[$k-1]['packageBuy'] == 3)
                        $arr[$k-1]['packageBuy'] == 2;
                }
        }       
    $sql = 'SELECT SUM(goods_number) AS number, SUM(goods_price * goods_number) AS amount' .
           ' FROM ' . $GLOBALS['ecs']->table('cart') .
           " WHERE session_id = '" . SESS_ID . "' AND rec_type = '" . CART_GENERAL_GOODS . "'";
    $row = $GLOBALS['db']->GetRow($sql);
//var_dump($GLOBALS['_CFG']['package_buy']);
    if ($row)
    {
        $number = intval($row['number']);
        ...
        $sql = 'SELECT  SUM(goods_price * goods_number) AS amount' .
           ' FROM ' . $GLOBALS['ecs']->table('cart') .
           " WHERE packageBuy_id != '0' AND session_id = '" . SESS_ID . "' AND rec_type = '" . CART_GENERAL_GOODS . "'";
          $discountMoney = (1 - floatval($GLOBALS['_CFG']['package_buy'])) * floatval($GLOBALS['db']->getOne($sql));
        $amount = floatval($row['amount']) - $discountMoney;
    }
    else
    {
        $number = 0;
    ...
    $GLOBALS['smarty']->assign('discount_rate',floatval($GLOBALS['_CFG']['package_buy']));
...
//var_dump($arr);
    ...
    $GLOBALS['smarty']->assign('amount',price_format($amount));
