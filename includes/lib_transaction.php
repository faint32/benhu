...
function get_profile($user_id)
{
    ...
    $info  = array();
    $infos = array();
    $sql  = "SELECT user_name,nickname,truename,bday,bname,bsex, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,".
    ...
    $info['user_name']   = $_SESSION['user_name'];  
	  ...
	  $info['nickname'] = isset($infos['nickname']) ? $infos['nickname'] : '';
	  $info['truename'] = isset($infos['truename']) ? $infos['truename'] : '';
$info['bday'] = isset($infos['bday']) ? $infos['bday'] : '';
		    $info['bname'] = isset($infos['bname']) ? $infos['bname'] : '';
			$info['bsex'] = isset($infos['bsex']) ? $infos['bsex'] : '';
 ...
 * @return  array   isfrist=1 为默认地址
 */
function get_consignee_list($user_id)
{
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_address') .
            " WHERE user_id = '$user_id'ORDER BY is_first DESC ";
      $consignee_list = $GLOBALS['db']->getAll($sql);
	 $consignee_list[0]['is_first'] = 1;  //如果没有默认收货地址，将第一个地址标记为默认地址。
...
function moren($id)
{
    ...
    {
        $sql = "UPDATE " .$GLOBALS['ecs']->table('user_address') . "SET is_first=1 WHERE address_id = '$id' ";
		$res = $GLOBALS['db']->query($sql);
        $GLOBALS['db']->query("UPDATE " .$GLOBALS['ecs']->table('user_address') . "SET is_first=0 WHERE address_id != '$id' ");

		$GLOBALS['db']->query("UPDATE " .$GLOBALS['ecs']->table('users') . "SET address_id='$id' WHERE user_id = $uid ");//该处是设置用户的默认地址guan
...
function update_address($address)
{
    ...
    {
		$sql = "select count(address_id) from ".$GLOBALS['ecs']->table('user_address').' where  user_id = ' . $address['user_id'];
	...
	if($num >= ADDRESS_NUM)
	{
	  return 2;
		...
		//$GLOBALS['db']->query("UPDATE " .$GLOBALS['ecs']->table('user_address') . "SET is_first=1 WHERE user_id = ". $address['user_id']." AND address_id='$address_id'");//guan-用户的默认地址

		$GLOBALS['db']->query("UPDATE " .$GLOBALS['ecs']->table('users') . "SET address_id='$address_id' WHERE user_id = ". $address['user_id']);//guan-用户的默认地址
    }
      if($address['is_first']==1){
		$GLOBALS['db']->query("UPDATE " .$GLOBALS['ecs']->table('users') . "SET address_id='$address_id' WHERE user_id = ". $address['user_id']);//guan-用户的默认地址
       $GLOBALS['db']->query("UPDATE " .$GLOBALS['ecs']->table('user_address') . "SET is_first=0 WHERE address_id != '$address_id' AND user_id = ". $address['user_id']);
      }
...
function noSpecialGoods($goods)
{
    foreach ($goods as $value) 
    {
        if( 0 != $value['packageBuy_id'] )
        {
            return false;
        }
        if(lowerPrice($value['goods_id'], $value['goods_price']) )
        {
            return false;
        }
    }
...
function lowerPrice($goodsId, $comparePrice=0)
{
    $shopPrice = $GLOBALS['db']->getOne("SELECT shop_price FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id='$goodsId'");
    if($comparePrice < $shopPrice)
        return true;

    return false;
}

function getCorrCoupons($userId, $restrictPrice)
{
    $sql = "SELECT sent_coupon_id, coupon_value, restriction_ext, coupon_description FROM ".$GLOBALS['ecs']->table('coupons_sent')." WHERE is_used = 0 AND user_id = '$userId' AND validate_time>".gmtime()." AND restriction_ext <= $restrictPrice ORDER BY coupon_value DESC";
