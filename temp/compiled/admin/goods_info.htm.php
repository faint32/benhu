      ...
      <form enctype="multipart/form-data" action="" method="post" name="theForm" >
        ...
        <table width="90%" id="general-table" align="center">
          <tr>
            ...
            <span id="goods_name_notice"></span>
            </td>
          </tr>
          <tr>
            <td class="label">
...
<!--            <td><input type="text" name="goods_number" value="<?php echo $this->_var['goods']['goods_number']; ?>" size="20" <?php if ($this->_var['code'] != '' || $this->_var['goods']['_attribute'] != ''): ?>readonly="readonly"<?php endif; ?> /><br />-->
		  ...
		  <tr>
            <td class="label">适用年龄：</td>
            <td>
                <select name="is_age">
                    <option value="0" <?php if (! $this->_var['goods']['is_age']): ?>selected<?php endif; ?>>请选择</option>
                    <option value="1" <?php if ($this->_var['goods']['is_age'] == 1): ?>selected<?php endif; ?> >0-3月</option>
                    <option value="2" <?php if ($this->_var['goods']['is_age'] == 2): ?>selected<?php endif; ?> >3-6月</option>
                    <option value="3" <?php if ($this->_var['goods']['is_age'] == 3): ?>selected<?php endif; ?> >6-12月</option>
                    <option value="4" <?php if ($this->_var['goods']['is_age'] == 4): ?>selected<?php endif; ?> >1-3岁</option>
                    <option value="5" <?php if ($this->_var['goods']['is_age'] == 5): ?>selected<?php endif; ?> >3岁以上</option>
                </select>
            </td>
          </tr>
		   <tr>
            <td class="label">优惠信息：</td>
            <td><textarea name="goods_yh" cols="40" rows="3"><?php echo htmlspecialchars($this->_var['goods']['goods_yh']); ?></textarea></td>
          </tr>
          <tr>
            <td class="label">
...
?>
  ...
  function validate(goods_id)
  {
      ...
      var goods_name  = document.forms['theForm'].elements['goods_name'].value;
      ...
      if (document.forms['theForm'].elements['goods_number'] != undefined)
      {
       ...
       var callback = function(resname)
     {
      if (resname.error > 0)
      {
        alert("<?php echo $this->_var['lang']['goods_name_exists']; ?>");
      }
      else
      {
         if(validator.passed())
    ...
    Ajax.call('goods.php?is_ajax=1&act=check_goods_name', "goods_name=" + goods_name + "&goods_id=" + goods_id, callback, "GET", "JSON");
...
?>
  ...
  * 判断商品名称是否重复
  */
  function checkGoodsName(goods_name, goods_id)
  {
    if (goods_name == '')
    {
        document.getElementById('goods_name_notice').innerHTML = "";
        return;
    }
    ...
    var callback = function(resname)
    {
      if (resname.error > 0)
      {
        document.getElementById('goods_name_notice').innerHTML = resname.message;
        document.getElementById('goods_name_notice').style.color = "red";
      }
      else
      {
        document.getElementById('goods_name_notice').innerHTML = "";
