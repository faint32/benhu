...
else if($_REQUEST['step'] == 'add_to_cart_packageBuy')
{
 include_once('includes/cls_json.php');
  include_once('includes/lib_sundries.php');
  
    $_POST['goodsArr']=strip_tags(urldecode($_POST['goodsArr']));
    $_POST['goodsArr'] = json_str_iconv($_POST['goodsArr']);
    ...
    if (empty($_POST['goodsArr']))
    {
        ...
        die($json->encode($result));
		   ...
		   $_POST['goodsArr'] = stripslashes($_POST['goodsArr']);   
	$goodsArr = json_decode($_POST['goodsArr']);
	
	checkPackageBuyValid($goodsArr, $result);

    /* 检查：如果商品有规格，而post的数据没有规格，把商品的规格属性通过JSON传到前台 */
	for($i = 1;;$i++)
	{
	$sql = 'select rec_id from '.$GLOBALS['ecs']->table('cart').' where packageBuy_id = '.$i;
    ...
    /* 更新：购物车 */
		...
		{
			// 更新：添加到购物车
			if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent,'', $packageBuy_id))
			{
				if ($_CFG['cart_confirm'] > 2)
				{
					$result['message'] = '';
...
//*/
    ...
    {
        // 更新：添加到购物车
		clear_cart(ART_GENERAL_GOODS,'buy_now');
	
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent,'buy_now'))
        {
            if ($_CFG['cart_confirm'] > 2)
            {
                $result['message'] = '';
	 ...
	 /* 计算合计 */
    $cart_goods = get_cart_goods('buy_now');
...
else if ($_REQUEST['step'] == 'buy_now_packageBuy')
{
   include_once('includes/cls_json.php');
   include_once('includes/lib_sundries.php');
...
//exit;
    /* 检查：如果商品有规格，而post的数据没有规格，把商品的规格属性通过JSON传到前台 */
		clear_cart(ART_GENERAL_GOODS,'buy_now');
			for($i = 1;;$i++)
			{
				$sql = 'select rec_id from '.$GLOBALS['ecs']->table('cart_buy_now').' where packageBuy_id = '.$i;
				if(!$GLOBALS['db']->getOne($sql))
				  break;
			}
			$packageBuy_id = $i;
    ...
    /* 更新：购物车 */
		else
		{
			...
			if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent,'buy_now',$packageBuy_id))
			{
				if ($_CFG['cart_confirm'] > 2)
				{
					$result['message'] = '';
...
elseif ($_REQUEST['step'] == 'checkout')
{
	...
	$smarty->assign('buyType',$buyType);
	 switch($buyType)
	 {
		case 'buy_now': $cart_table = 'cart_buy_now'; break;
		default: $cart_table = 'cart'; break;
	 }
    $sql = "SELECT COUNT(*) FROM " . $ecs->table($cart_table) .
	...
	if ($action == 'moren')
	{
	    ...
	    if (moren($consignee_id))
	    {
			set_session_consignee($smarty);
			  ...
			  $goods_list = $_REQUEST['goods_list'];//chen-0926
	          $goods_list = explode(',',$goods_list);
	
	         $buyType = ($_REQUEST['buyType'] == 'buy_now') ? 'buy_now' : '';

	
              $cart_goods = cart_goods($flow_type , $goods_list, $buyType); // 取得商品列表，计算合计
	
			   $consignee1 = get_consignee($_SESSION['user_id']);

			  $total = order_fee($order, $cart_goods, $consignee1);
			echo json_encode( array('consignee'=>$consignee1, 'shippingFee'=>$total['shipping_fee_formated']) );
	    }
	    else
	    {
	        echo json_encode('0');
    ...
    $_SESSION['flow_consignee'] = $consignee1;
   
    $smarty->assign('consignee1', $consignee1);
    ...
    $packageBuyDiscountMoney = price_format(packageBuyDiscountMoney($cart_goods));
    ...
    {
		foreach($cart_goods as $cart_goodsKey => $cart_goodsVal)
		{
		 $cart_goods[$cart_goodsKey]['goods_price'] =   $cart_goods[$cart_goodsKey]['needed_money'];
		}
    ...
    /* 取得配送列表 */
    $region            = array($consignee1['country'], $consignee1['province'], $consignee1['city'], $consignee1['district']);
    ...
    if ($flow_type == CART_GENERAL_GOODS && noSpecialGoods($cart_goods))
    {
        // 能使用购物券
        $coupons = getCorrCoupons($_SESSION['user_id'], $total['goods_price']);
        // var_dump($coupons);
...
elseif ($_REQUEST['step'] == 'change_coupons')
{
    ...
    else
    {
        ...
        if (!empty($coupons) && $coupons['user_id'] == $_SESSION['user_id'])
        {
...
elseif ($_REQUEST['step'] == 'change_needinv')
{
    ...
    if (empty($cart_goods) || !check_consignee_info($consignee, $flow_type))
    {
      //  $result['error'] = $_LANG['no_goods_in_cart'];
      //  die($json->encode($result));
	  die($json->encode('no_goods_in_cart'));
    }
    else
    {
        /* 取得购物流程设置 */
		...
		$tmp = array('need_inv'=>$order['need_inv'],'tax_formated'=>$total['tax_formated'],'amount'=>$total['amount']);
         //die($json->encode($total['tax_formated'],$total['amount']));
		 die($json->encode($tmp));
    }
...
elseif ($_REQUEST['step'] == 'check_integral')
{
  ...
  /*if ($points > $flow_points)
    {
        die("这个积分商品您最多可以使用".$flow_points."积分");
    }*/
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    {
        $cart_goods_stock = get_cart_goods($buyType);
    ...
    if ($order['coupons_id'] > 0)
    {
        ...
        if (empty($coupons) || $coupons['user_id'] != $user_id || $coupons['order_id'] > 0 || floatval($coupons['restriction_ext']) > cart_amount(true, $flow_type))
        {
            $order['coupons_id'] = 0;
        }
    }
    ...
    /* 订单中的商品 */
    $cart_goods = cart_goods($flow_type,'',$buyType);
   ...
   /* 判断是不是实体商品 */
    ...
    // 购物车中的商品能享受红包支付的总额
    $discount_amout = 0;//compute_discount_amount();
    // 红包和积分最多能支付的金额为商品总额
    ...
    /* 插入订单商品 */
	$sql = '';
	if($flow_type == CART_EXCHANGE_GOODS) //chen-1015 积分购，goods_price采用needed_money
	{
	  
        $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
                "order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, ".
                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, packageBuyId,packageBuyDiscount) ".
            " SELECT '$new_order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, ".
                "needed_money, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, packageBuy_id,".getPackageBuyDiscount().
            " FROM " .$ecs->table($cart_table) .
	...
	else 
	{
	  $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
                ...
                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, packageBuyId,packageBuyDiscount) ".
            " SELECT '$new_order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, ".
                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id, packageBuy_id,".getPackageBuyDiscount().
            " FROM " .$ecs->table($cart_table) .
            " WHERE session_id = '".SESS_ID."' AND rec_type = '$flow_type'";
	}
	$db->query($sql);
      ...
      require_once(ROOT_PATH . 'includes/lib_integral.php');
	    ...
	    $integral_point=integral_to_give(order_info($order['order_id']));
	   freeze_integral((int)($integral_point['custom_points']),$order['user_id'],0,"订单未完成，积分冻结",$freeze_type='order',$order['order_sn']);
	// echo 'qa123';exit;
    /* 修改拍卖活动状态 */
   ...
   if ($order['user_id'] > 0 && $order['integral'] > 0)
    ...
    if ($order['order_amount'] > 0)
    {
        ...
        $smarty->assign('pay_code', $payment['pay_code']); //采用哪种支付方式
...
elseif ($_REQUEST['step'] == 'immediate_pay') //从我的订单页过来，展示的页面和step=done一样
{
	...
	$order = order_info($_REQUEST['order_id']);
      $_REQUEST['step'] = 'done';
		 ...
		 include_once('includes/lib_payment.php');
		 $payment = payment_info(DEFAULT_PAYID);
...
elseif ($_REQUEST['step'] == 'ajax_check_pay') //step=done页更改支付方式
{
 include_once('includes/lib_payment.php');
...
elseif ($_REQUEST['step']== 'ajax_update_cart')//动态更新购物车的商品数量
{
...
elseif($_REQUEST['step'] == 'ajax_taocan_update_cart')
{
 require_once(ROOT_PATH .'includes/cls_json.php');
     $json = new JSON();
     $result = array('error' => 0, 'message'=> '');
     if (isset($_POST['packageBuy_id']) && isset($_POST['goods_number']))
     {
		$key = $_POST['packageBuy_id'];
		$val = $_POST['goods_number'];
		$result['packageBuy_id'] = $key;
		$val = intval(make_semiangle($val));
		if ($val <= 0 && !is_numeric($key))
		{
			$result['error'] = 99;
		...
		$sql = "SELECT `goods_id`, `goods_attr_id`,`product_id`, `extension_code` FROM" .$GLOBALS['ecs']->table('cart')." WHERE packageBuy_id='$key' AND session_id='" . SESS_ID . "'";
		$goods =$GLOBALS['db']->getAll($sql);
		
		$sql = "SELECT g.goods_name,g.goods_number "."FROM ".$GLOBALS['ecs']->table('goods'). " AS g, ".$GLOBALS['ecs']->table('cart'). " AS c "."WHERE g.goods_id =c.goods_id AND c.packageBuy_id = '$key'";
		...
		{
		  	//查询：系统启用了库存，检查输入的商品数量是否有效
			if(intval($GLOBALS['_CFG']['use_storage']) > 0 && $goods[$i]['extension_code'] != 'package_buy')
			{
				/*是货品*/
				$goods[$i]['product_id'] = trim($goods[$i]['product_id']);
				if (!empty($goods[$i]['product_id']))
				{
					$sql = "SELECT product_number FROM " .$GLOBALS['ecs']->table('products'). " WHERE goods_id = '" . $goods[$i]['goods_id'] . "' AND product_id = '" .$goods[$i]['product_id'] . "'";
					$product_number = $GLOBALS['db']->getOne($sql);
		
					if ($product_number < $val)
					{
						$result['error'] = 2;
						$result['message'] =sprintf($GLOBALS['_LANG']['stock_insufficiency'], $row[$i]['goods_name'],$product_number, $product_number);
						$result['goods_number'] = $product_number;
						die($json->encode($result));
					}
				}
				...
				{
					if ($row[$i]['goods_number'] < $val)
					{
						$result['error'] = 1;
						$result['goods_number'] = $row[$i]['goods_number'];
						$result['message'] =sprintf($GLOBALS['_LANG']['stock_insufficiency'], $row[$i]['goods_name'],$row[$i]['goods_number'], $row[$i]['goods_number']);
						die($json->encode($result));
					}
				}
			}
			elseif (intval($GLOBALS['_CFG']['use_storage']) > 0 && $goods[$i]['extension_code'] == 'package_buy')
			{
				if(judge_package_stock($goods[$i]['goods_id'], $val))
				{
					$result['error'] = 3;
					$result['message'] =$GLOBALS['_LANG']['package_stock_insufficiency'];
					die($json->encode($result));
		...
		{
				//更新购物车中的商品数量
				$sql = "UPDATE ".$GLOBALS['ecs']->table('cart')." SET goods_number= '$val' WHERE packageBuy_id='$key' AND session_id='" . SESS_ID . "'";
		}
		else
		{
		  	$sql = "DELETE FROM ".$GLOBALS['ecs']->table('cart')." WHERE packageBuy_id='$key' AND session_id='" .SESS_ID. "'";
		}
		$GLOBALS['db']->query($sql);
		/*删除所有赠品*/
		$result['packageBuy_id'] = $key;
		$result['goods_number'] = $val;
		$result['goods_subtotal'] = 0;
		$result['total_desc'] = '';
...
{
	if(isset($_REQUEST['buyType']) && $_REQUEST['buyType'] == 'taocan')
	{
	  $packageBuy_id = $_GET['id'];
	  $sql = "delete  from ".$GLOBALS['ecs']->table('cart')." where packageBuy_id = '$packageBuy_id'";
	 if(!$GLOBALS['db']->query($sql))
	 {
	     show_message('删除失败', '返回', 'flow.php' ,  'info',  true);
	 }
	  ecs_header("Location: flow.php\n");
     exit;
	}
	...
	for($i = 0; $i < count($goods_list); $i++)
	{
      ...
      flow_drop_cart_goods(intval($goods_list[$i]));
    }
   ecs_header("Location: flow.php\n");
    exit;
}
...
elseif ($_REQUEST['step'] == 'drop_to_collect')
{
	...
	$goods_list = explode(',' , $goods_list);
	  ...
	  for($i = 0; $i < count($goods_list); $i++)
	   {
	   $rec_id = intval($goods_list[$i]);
	   if(!$rec_id)
	   {
	    show_message($content="收藏失败，您没有选择要收藏的商品！", $links = '点击返回', $hrefs = 'flow.php', $type = 'info', $auto_redirect = true);
	    exit;
	   }
        ...
        if (empty($count))
        {
		   ...
		   if(!$flag)
		   {
		   show_message($content="收藏失败，您可以重新尝试一下！", $links = '点击返回', $hrefs = 'flow.php', $type = 'info', $auto_redirect = true);
		   exit;
		   }
        }
      
		}
		  show_message($content="收藏成功！", $links = '点击返回', $hrefs = 'flow.php', $type = 'info', $auto_redirect = true);
		 exit;
    }
	else
	{
	show_message($content="您没有登录不能收藏商品", $links = '点击返回', $hrefs = 'flow.php', $type = 'info', $auto_redirect = true);
	 exit;
	}
...
//猜你喜欢 随机出   标记有精品的商品is_best=1
    $cai = $db->getALL('SELECT goods_id,goods_name,market_price,shop_price,goods_thumb FROM ' . $ecs->table("goods") . ' WHERE is_best=1 ORDER BY RAND() LIMIT 0,6 ');
    $smarty->assign('cai', $cai);
//购买该商品的人还喜欢
   /* echo $goods_id."<br>===";
    $per=$db->getALL('SELECT user_id FROM ' . $ecs->table("order_info") .'AS i,'.$ecs->table("order_goods") .'AS o,'. $ecs->table("goods") .  'AS g ON g.goods_id=o.goods_id AND g.order_id=i.order_id WHERE g.goods_id='.$goods_id );
    $userids=  explode(',', $per);
    print_r($userids);
    $like=$db->getALL('SELECT g.goods_id,g.goods_name,g.market_price,g.shop_price,g.goods_thumb FROM '  . $ecs->table("order_info") .'AS i,'.$ecs->table("order_goods") .'AS o,'. $ecs->table("goods") .  'AS g ON g.goods_id=o.goods_id AND g.order_id=i.order_id AND i.user_id in ($userids) ORDER BY RAND() LIMIT 1 ');
*/
...
function get_cart_show_goods($goods_id){
	...
	$goods_html = "<div class='blank'></div>";
	$goods_html .= "<span class='close' ><a href='javascript:close_message_div()'>关闭</a></span>";
	$goods_html .= "<div class='ok'></div><h4>宝贝已成功添加到购物车！</h4>";
    $goods_html .= "<p>购物车共有<strong>".$cart_info['number']."</strong>件宝贝，合计：<strong class='price'>".$cart_info['amount']."</strong>元</p>";
    $goods_html .= "<a  href='flow.php?act=cart'  title='去购物车结算'><img style='margin:10px 50px;' src='./themes/quwan/images/gocart.gif'></a>";
    $goods_html .= "<a  href='javascript:close_message_div()'  title='再逛逛'><img style='margin:10px 20px;' src='./themes/quwan/images/zgg.gif'></a>";
	$goods_html .="<div class='othersbuy'>购买过此产品的用户还购买了：</div>";
   
	foreach($show_goods as $row)
	{
	    $goods_html .= "<div class='goodsItem'>";
		$goods_html .= '<a href="goods.php?id=203">';
		$goods_html .= '<img src="'.$row['goods_thumb'].'"  class="goodsimg"></a>';
        $goods_html .= '<p class="name"><a href="'.build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']).'" title="'.$row['goods_name'].'">"'.$row['goods_name'].'"</a></p>';
        $goods_html .= '<p class="price">';
		$goods_html .= '<font class="shop">售价：￥'.$row['shop_price'].'</font></p>';
	    $goods_html .= "</div>";
	}
	return $goods_html;
}
